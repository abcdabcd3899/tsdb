...
/*
 * key structure for "Motion Parallel Hash"
 */
typedef struct MotionParallelKey
{
	int session_id;
	int command_id;
	int plan_node_id;
...
typedef struct MotionParallelEntry
{
	MotionParallelKey	key;
	dsm_handle	dsm_handler;
	Barrier		barrier;
	int			nparticipants;			/* the number of expected participants */
	volatile pg_atomic_uint32	nattached;		/* the number of attached participants */
	volatile pg_atomic_uint32	ndetached;		/* the number of detached participants */
} MotionParallelEntry;

typedef struct MotionParallelShmem
{
	HTAB	*hash;
} MotionParallelShmem;

extern void MotionParallelShmemInit(void);
