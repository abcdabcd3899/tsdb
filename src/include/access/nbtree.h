...
# define USED_FOR_ODIN_ONLY

# define ODIN_FEATURE_BEGIN(feature, indexoid) \
	if ((odin_features & ODIN_FEATURE_ ## feature) && \
		odin_indexoid == (indexoid)) \
	{
# define ODIN_FEATURE_OTHERWISE(...) \
	} else {
# define ODIN_FEATURE_END(...) \
	}

extern Oid	odin_indexoid;
extern unsigned int odin_features;
# define ODIN_FEATURE_INDEXSCAN		0x00000001
# define ODIN_FEATURE_BITMAPSCAN	0x00000002
# define ODIN_FEATURE_ALL			0xffffffff

#else  /* ENABLE_ODIN */

# define USED_FOR_ODIN_ONLY pg_attribute_unused()

# define ODIN_FEATURE_BEGIN(feature, indexoid) \
	if (0) \
	{
# define ODIN_FEATURE_OTHERWISE(feature) \
	} else {
# define ODIN_FEATURE_END(feature) \
	}

#endif  /* ENABLE_ODIN */

#define OdinGetFillFactor(relation) \
	((relation)->rd_options ? \
	 ((OdinOptions *) (relation)->rd_options)->fillfactor : \
	  ODIN_DEFAULT_FILL_FACTOR)
#define OdinGetTuplesPerRange(relation) \
	((relation)->rd_options ? \
	 ((OdinOptions *) (relation)->rd_options)->tuples_per_range : \
	  ODIN_DEFAULT_TUPLES_PER_RANGE)
#define OdinGetBitmapScan(relation) \
	((relation)->rd_options ? \
	 ((OdinOptions *) (relation)->rd_options)->bitmap_scan : \
	  ODIN_DEFAULT_BITMAP_SCAN)

typedef struct OdinOptions
{
	int32		vl_len_;		/* varlena header (do not touch directly!) */

	int			fillfactor;
#define ODIN_MIN_FILL_FACTOR 10
#define ODIN_MAX_FILL_FACTOR 100
#define ODIN_DEFAULT_FILL_FACTOR ODIN_MAX_FILL_FACTOR

	int			tuples_per_range;
#define ODIN_AUTO_TUPLES_PER_RANGE 0
#define ODIN_MIN_TUPLES_PER_RANGE 0
#define ODIN_MAX_TUPLES_PER_RANGE 32768
#define ODIN_DEFAULT_TUPLES_PER_RANGE ODIN_AUTO_TUPLES_PER_RANGE

	bool		bitmap_scan;
#define ODIN_DEFAULT_BITMAP_SCAN true
} OdinOptions;
...
typedef struct BTScanOpaqueData
{
	...
	/*
	 * Below attributes are for ODIN.
	 */

	IndexFetchTableData *odin_heapfetch;
	TupleTableSlot *odin_slot;	/* slot for heap scan */
	bool		odin_counted;
	bool		odin_advancing;	/* true if advancing in parallel scan */
	BlockNumber	odin_firstBlock;
	BlockNumber	odin_lastBlock;
	BlockNumber	odin_currentBlock;
	OffsetNumber odin_minoff;
	OffsetNumber odin_maxoff;

	BTScanInsertData odin_inskey;	/* remember inskey */
} BTScanOpaqueData;
...
/*
 * prototypes for functions in odinheap.c
 */
extern List *odin_make_tlist_from_scankeys(IndexScanDesc scan);
extern Relation odin_get_heap_relation(IndexScanDesc scan);
extern bool odin_get_heap_tuple(IndexScanDesc scan,
								BlockNumber blkno, OffsetNumber off);
extern void odin_get_heap_nblocks(IndexScanDesc scan);
extern OffsetNumber odin_get_heap_block_ntuples(IndexScanDesc scan,
												BlockNumber blkno,
												OffsetNumber *firstOff,
												OffsetNumber *lastOff);
extern int32 odin_compare_heap(IndexScanDesc scan, BTScanInsert key);
extern OffsetNumber odin_binsearch_heap(IndexScanDesc scan,
										ItemPointer first_tid);
extern bool odin_first_match(IndexScanDesc scan, ScanDirection dir,
							 BlockNumber h_blkno, OffsetNumber h_off);
extern BlockNumber odin_parallel_advance_block(IndexScanDesc scan,
											   ScanDirection dir,
											   BlockNumber h_blkno);
