...
typedef struct TableAmRoutine
{
	...
	/*
	 * GPDB_12_MERGE_FIXME: Extract columns for fetch from targetlist and quals.
	 */
	void		(*index_fetch_extractcolumns) (struct IndexFetchTableData *scan,
											   List *targetlist,
											   List *qual);

#ifdef MATRIXDB_VERSION
	/*
	 * MatrixDB private callback
	 */
	void (*event_notify) (Relation relation, CmdType operation, void *param);

	/*
	 * MatrixDB private contenxt
	 */
	void	*priv;
 ...
 */
static inline TableScanDesc
table_beginscan_analyze(Relation rel, List *va_cols)
{
...
extern TableScanDesc table_beginscan_parallel_es(Relation relation,
												 List *targetList, List *qual,
...
/*
 * Extract the columns from the targetlist and qual.
 *
 * This is useful for column storage like AOCS.
 */
static inline void
table_index_fetch_extractcolumns(struct IndexFetchTableData *scan,
								 List *targetlist,
								 List *qual)
{
	if (scan->rel->rd_tableam->index_fetch_extractcolumns)
		scan->rel->rd_tableam->index_fetch_extractcolumns(scan,
														  targetlist,
														  qual);
}
...
#ifdef MATRIXDB_VERSION
static inline void
table_event_notify(Relation relation, CmdType operation, void* param)
{
	if (relation &&
		relation->rd_tableam &&
		relation->rd_tableam->event_notify &&
		/* only trigger the notify on real tables */
		relation->rd_rel->relkind != RELKIND_INDEX &&
		relation->rd_rel->relkind != RELKIND_PARTITIONED_INDEX &&
		relation->rd_rel->relkind != RELKIND_COMPOSITE_TYPE)
		relation->rd_tableam->event_notify(relation, operation, param);
}
#endif /* MATRIXDB_VERSION */
