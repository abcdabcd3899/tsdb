...
bool 		mpp_parallel_enabled = false;
bool 		legacy_parallel_enabled = false;
bool 		gdd_enabled = false;
...
get_expectfile(const char *testname, const char *file, const char *default_expectfile)
{
	...
	if (legacy_parallel_enabled)
	{
		snprintf(buf, sizeof(buf), "%s/%s_legacy_parallel.%s", expectpath, base_file, file_type);
		if (file_exists(buf))
			return strdup(buf);
	}
	if (gdd_enabled)
	{
		snprintf(buf, sizeof(buf), "%s/%s_gdd.%s", expectpath, base_file, file_type);
		if (file_exists(buf))
			return strdup(buf);
	}
	if (mpp_parallel_enabled)
	{
		snprintf(buf, sizeof(buf), "%s/%s_mpp_parallel.%s", expectpath, base_file, file_type);
		if (file_exists(buf))
			return strdup(buf);
	}
...
#define ULONGPID(x) (unsigned long) (x)
	...
	mpp_parallel_enabled = check_feature_status("enable_parallel_mode", "on",
			"Parallel mode enabled. Using parallel answer files whenever possible",
			"Parallel mode disabled. Using parallel answer files");

	gdd_enabled = check_feature_status("gp_enable_global_deadlock_detector", "on",
			"GDD mode enabled. Using GDD answer files whenever possible",
			"GDD mode disabled. Using GDD answer files");

	legacy_parallel_enabled = mpp_parallel_enabled &&
		check_feature_status("parallel_setup_cost", "0", "", "") &&
		check_feature_status("parallel_tuple_cost", "0", "", "");
