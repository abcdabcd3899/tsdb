...
{
	bool should_sent_local = false;
	bool should_sent_shared = false;
	int32 shared_next_typmod = SharedRecordNextTypmod(); 

	should_sent_local = NextRecordTypmod > 0 &&
		NextRecordTypmod > conn->sent_record_typmod;

	should_sent_shared =
		shared_next_typmod > 0 &&
		shared_next_typmod > conn->sent_shared_record_typmod;

	return pSerInfo->has_record_types &&
		(should_sent_shared || should_sent_local);
}
...
UpdateSentRecordCache(MotionConn *conn)
{
	...
	conn->sent_record_typmod = SharedRecordNextTypmod();;
}
