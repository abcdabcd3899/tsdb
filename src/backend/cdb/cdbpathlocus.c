...
cdbpathlocus_equal(CdbPathLocus a, CdbPathLocus b)
{
	...
	if (cdbpathlocus_distkey_equal(a, b))
		return true;
...
bool
cdbpathlocus_distkey_equal(CdbPathLocus a, CdbPathLocus b)
{
	ListCell   *acell;
	ListCell   *bcell;
	ListCell   *a_ec_cell;
 ...
 */
CdbPathLocus
cdbpathlocus_from_policy(struct PlannerInfo *root, Index rti,
						 GpPolicy *policy,
...
cdbpathlocus_join(JoinType jointype, CdbPathLocus a, CdbPathLocus b)
{
	...
	if (cdbpathlocus_distkey_equal(a, b))
	{
		/* We get here only when a partial path join with a non-partial path */
		Assert(a.hashflag == b.hashflag);
		Assert((a.parallel_workers == 0 && b.parallel_workers > 0) ||
			   (a.parallel_workers > 0 && b.parallel_workers == 0));

		/* join path has same locus with path with larger workers */
		return a.parallel_workers > b.parallel_workers ? a : b;
	}
								   ...
								   bool ignore_constants)
{
	...
	if (CdbPathLocus_IsHashed(locus) || CdbPathLocus_IsHashedOJ(locus))
	{
		...
		if (!cdbpathlocus_is_hash_distkey_valid(locus))
			return false;
...
cdbpathlocus_is_valid(CdbPathLocus locus)
{
	...
	if (CdbPathLocus_IsHashed(locus) || CdbPathLocus_IsHashedOJ(locus))
	{
		...
		if (!IsA(locus.distkey, List))
			goto bad;
		/* non parallel locus always has HASHED_ON_SEGMENT */
		if (locus.parallel_workers == 0 && locus.hashflag != HASHED_ON_SEGMENT)
			goto bad;
		/* parallel locus is either HASHED_ON_SEGMENT or HASHED_ON_WORKERS */
		if (locus.parallel_workers > 0 &&
			locus.hashflag == (HASHED_ON_SEGMENT | HASHED_ON_WORKERS))
			goto bad;
		foreach(distkeycell, locus.distkey)
		{
 ...
 */
bool
cdbpathlocus_is_hash_distkey_valid(CdbPathLocus locus)
{
	Assert(CdbPathLocus_IsHashed(locus) || CdbPathLocus_IsHashedOJ(locus));

	if (locus.parallel_workers > 0 &&
		locus.hashflag == HASHED_ON_SEGMENT)
		return false;
