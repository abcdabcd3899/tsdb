...
static char *buildGpQueryString(DispatchCommandQueryParms *pQueryParms,
				   int *finalLen, MemoryContext context);
...
CdbDispatchSetCommand(const char *strCommand, bool cancelOnError)
{
	...
	queryText = buildGpQueryString(pQueryParms, &queryTextLength, DispatcherContext);
...
char *
CdbCreateMPPQueryText(const char *query_string, int flags, int *query_text_len)
{
	DispatchCommandQueryParms	*query_parms;
	...
	query_parms = cdbdisp_buildCommandQueryParms(query_string, flags);

	query_text = buildGpQueryString(query_parms, query_text_len, NULL);
				   ...
				   int *finalLen, MemoryContext context)
{
	...
	/*
	 * allocate query text in specified MemoryContext, normally be DispatcherContext,
	 */
	if (context)
	 ...
	 */
	if (!command)
		command = "unknown query (a utility statement maybe)";
						   ...
						   Bitmapset *sendParams)
{
	 ...
	 */
	if (found_records)
		result->transientTypes = build_tuple_node_list(0, 0);
