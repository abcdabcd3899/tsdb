...
cdbpath_parallel_workers(Path *path)
{
	if (path->parallel_workers)
		return CdbPathLocus_NumSegments(path->locus) *
...
cdbpath_cost_motion(PlannerInfo *root, CdbMotionPath *motionpath)
{
	...
	if (CdbPathLocus_IsPartitioned(motionpath->path.locus))
		recv_segments = cdbpath_parallel_workers(&motionpath->path);
	else
		recv_segments = 1;

	if (CdbPathLocus_IsPartitioned(subpath->locus))
		send_segments = cdbpath_parallel_workers(subpath);
	else
		send_segments = 1;
	...
	motion_cost = cost_per_row * 0.5 * (sendrows + recvrows);

	run_cost += motion_cost;

	if (subpath->parallel_workers)
		startup_cost += gp_motion_cost_startup * subpath->parallel_workers;
	else
		startup_cost += gp_motion_cost_startup;
									 ...
									 int numsegments)
{
	...
	/* Motion doesn't project, so use source path's pathtarget */
	pathnode->path.pathtarget = subpath->pathtarget;
	CdbPathLocus_MakeReplicated(&pathnode->path.locus, numsegments, 0);
	pathnode->path.rows = subpath->rows;
	pathnode->path.pathkeys = NIL;
									 ...
									 CdbPathLocus inner_locus)
{
	...
	if (!mergeclause_list ||
		...
		outer_locus.hashflag != inner_locus.hashflag)
		return false;
	...
	{
		/* HASHED_ON_WORKERS must have same parallel_workers */
		if (outer_locus.parallel_workers != inner_locus.parallel_workers)
			return false;
	}
	else
	{
		/* HASHED_ON_SEGMENT is invalid when both paths are non-parallel */
		if (outer_locus.parallel_workers > 0 && inner_locus.parallel_workers > 0)
			return false;
	}
						  ...
						  GpPolicy *policy)
{
	...
	{
		/* the target table is always thought with 0 parallel workers */
