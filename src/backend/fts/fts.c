...
/*
 * MatrixDB: SIGTERM: set flag to indicate FTS is terminated.
 *
 * Derived from bgworker_die(), the original SIGTERM handler.
 */
static void
sigTermHandler(SIGNAL_ARGS)
{
	...
	/* The flag is only checked by QDs, so we set it in the shm directly */
	if (ftsProbeInfo)
		ftsProbeInfo->terminated = true;
...
FtsProbeMain(Datum main_arg)
{
	*shmFtsProbePID = MyProcPid;
	...
	/*
	 * MatrixDB: when terminating the cluster with command "gpstop -a", the
	 * postmaster will reject any new session, terminate bgworkers, including
	 * the FTS, and waiting for existing sessions to quit.  However if an
	 * existing session tries to execute a query that needs an entry db, such
	 * as "insert into t1 (c1) select oid from pg_class", then it will fail
	 * because entry dbs are also rejected by the postmaster, thus it will run
	 * into the exception handling logic in cdbgang_createGang_async(), which
	 * calls FtsNotifyProber() and wait for FTS to refresh the segment
	 * configuration, however as FTS is already terminated, the QD will run
	 * into endless waiting.
	 *
	 * To prevent that, the FTS sets a flag in the shm on SIGTERM, so the QDs
	 * know about it.
	 */
