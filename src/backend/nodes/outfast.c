...
static void
_outGpAcquireSampleRowsStmt(StringInfo str, const GpAcquireSampleRowsStmt *node)
{
	WRITE_NODE_TYPE("GPACQUIRESAMPLEROWSSTMT");

	WRITE_OID_FIELD(relOid);
	WRITE_NODE_FIELD(va_cols);
	WRITE_INT_FIELD(targrows);
	WRITE_BOOL_FIELD(inherited);
}
	...
	else
	{
		switch (nodeTag(obj))
		{
			...
			case T_GpAcquireSampleRowsStmt:
				_outGpAcquireSampleRowsStmt(str, obj);
				break;
