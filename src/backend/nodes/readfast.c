...
_readOidAssignment(void)
{
	...
	READ_OID_FIELD(oid);
	if (local_node->objname == NULL)
		local_node->objname = pstrdup("");
	READ_DONE();
}
...
_readMotion(void)
{
	...
	READ_INT_FIELD(sender_parallel_workers);
...
_readGpAcquireSampleRowsStmt(void)
{
	READ_LOCALS(GpAcquireSampleRowsStmt);
	READ_OID_FIELD(relOid);
	READ_NODE_FIELD(va_cols);
	READ_INT_FIELD(targrows);
	READ_BOOL_FIELD(inherited);
	READ_DONE();
}
...
readNodeBinary(void)
{
	...
	switch(nt)
	{
			...
			case T_GpAcquireSampleRowsStmt:
				return_value = _readGpAcquireSampleRowsStmt();
				break;
#ifdef MATRIXDB_VERSION
