...
_copyMotion(const Motion *from)
{
	...
	COPY_SCALAR_FIELD(sender_parallel_workers);
...
static GpAcquireSampleRowsStmt*
_copyGpAcquireSampleRowsStmt(const GpAcquireSampleRowsStmt *from)
{
	GpAcquireSampleRowsStmt *newnode =
		makeNode(GpAcquireSampleRowsStmt);

	COPY_SCALAR_FIELD(relOid);
	COPY_NODE_FIELD(va_cols);
	COPY_SCALAR_FIELD(targrows);
	COPY_SCALAR_FIELD(inherited);

	return newnode;
}

#ifdef MATRIXDB_VERSION
static MarsDimensionAttr *
_copyMarsDimensionAttr(const MarsDimensionAttr *from)
{
	MarsDimensionAttr *newnode = makeNode(MarsDimensionAttr);

	COPY_SCALAR_FIELD(dimensiontype);
	COPY_SCALAR_FIELD(attnum);
	COPY_SCALAR_FIELD(param);
...
copyObjectImpl(const void *from)
{
	...
	switch (nodeTag(from))
	{
		...
		case T_GpAcquireSampleRowsStmt:
			retval = _copyGpAcquireSampleRowsStmt(from);
			break;

#ifdef MATRIXDB_VERSION
		case T_MarsDimensionAttr:
			retval = _copyMarsDimensionAttr(from);
			break;
