						...
						bool needsWAL)
{
	...
	if (acc->ao_attr.compress)
	{
		 ...
		 * BULK compression.
		 */
		compressionFunctions = mx_get_shared_compression_functions(acc->ao_attr.compressType);
		if (compressionFunctions != NULL)
		{
			TupleDesc	td = CreateTupleDesc(1, &attr);

			compressionState =
				mx_get_shared_compression_state(true /* compress */,
			...
			if (gp_appendonly_verify_write_block)
				verifyBlockCompressionState =
					mx_get_shared_compression_state(false /* decompress */,
													acc->ao_attr.compressType,
													acc->ao_attr.compressLevel,
													acc->maxAoBlockSize,
													td);
...
retry:
	...
	if (rs_parallel != NULL)
	{
		ParallelBlockTableScanDesc pbscan =
			(ParallelBlockTableScanDesc) rs_parallel;

		/* the first attr */
		if (colIndex == 0)
		{
			acc->ao_read.ao_parallel_scan.current++;

			/* sync with other workers to get next block to fetch */
			if (acc->ao_read.ao_parallel_scan.target == 0)
			{
				table_block_parallelscan_startblock_init(NULL, pbscan);
				acc->ao_read.ao_parallel_scan.target =
					table_block_parallelscan_nextpage(NULL, pbscan);
			}

			/* skip current block if it is not the target block */
			if (acc->ao_read.ao_parallel_scan.current <
				acc->ao_read.ao_parallel_scan.target + 1)
			{
				AppendOnlyStorageRead_SkipCurrentBlock(&acc->ao_read);
				goto retry;		
			}

			/* reset parallel target block */
			acc->ao_read.ao_parallel_scan.target = 0;
		}
		/* following attrs */
		else if (rowNum > 0)
		{
			Assert(rowNum >= acc->blockFirstRowNum);

			if (rowNum > acc->blockFirstRowNum + acc->blockRowCount - 1)
			{
				AppendOnlyStorageRead_SkipCurrentBlock(&acc->ao_read);
				goto retry;		
			}
	...
	if (rs_parallel != NULL && colIndex > 0 && rowNum > 0)
	{
		int current = acc->blockFirstRowNum;

		while(rowNum  > current)
		{
			datumstreamread_advance(acc);
			current++;
		}
	}
...
datumstreamread_find(DatumStreamRead * datumStream,
					 int32 rowNumInBlock)
{
	DatumStreamBlockRead *dsr = &datumStream->blockRead;

	/*
	 * If the position was already visited we could seek with the cached
	 * datums, however we lack the knowledge to restore all the fields, so we
	 * seek to the previous datum, then let datumstreamread_advance() fill the
	 * fields for us.  This requires one extra advance every time, but it is
	 * already a huge optimization compared to a full reset.
	 *
	 * TODO: support for large object.
	 */
	if (rowNumInBlock > 0 && rowNumInBlock < dsr->cache.count)
	{
		typeof(dsr->cache.records[0]) *record;

		dsr->nth = rowNumInBlock - 1;
		record = &dsr->cache.records[dsr->nth];

		/* the only thing we can restore is the datum position */
		dsr->physical_datum_index = record->physical_datum_index;
		dsr->datump = record->datump;
		if (dsr->has_null)
		{
#ifdef USE_ASSERT_CHECKING
			dsr->null_bitmap = record->null_bitmap;
#else /* USE_ASSERT_CHECKING */
			DatumStreamBitMapRead_Seek(&dsr->null_bitmap, dsr->nth);
	...
	while (rowNumInBlock > DatumStreamBlockRead_Nth(&datumStream->blockRead))
	{
		...
		if (dsr->nth >= dsr->cache.count)
		{
			typeof(dsr->cache.records[0]) *record;

			Assert(dsr->nth == dsr->cache.count);

			record = &dsr->cache.records[dsr->cache.count++];

			record->datump = dsr->datump;
			record->physical_datum_index = dsr->physical_datum_index;
			if (dsr->has_null)
			{
#ifdef USE_ASSERT_CHECKING
				record->null_bitmap = dsr->null_bitmap;
#else /* USE_ASSERT_CHECKING */
				Assert(dsr->null_bitmap.bitPosition == dsr->bitPosition);
#endif /* USE_ASSERT_CHECKING */
			}
		}
	}
