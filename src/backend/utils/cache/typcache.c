...
int32
SharedRecordNextTypmod(void)
{
	if (CurrentSession->shared_typmod_registry != NULL) 
		return pg_atomic_read_u32(&CurrentSession->shared_typmod_registry->next_typmod);
	else
		return 0;
}
 ...
 */
List *
build_tuple_node_list(int local_start, int shared_start)
{
	...
	/* Also build record type in shared registers if any */
	i = shared_start;
	if (CurrentSession->shared_typmod_registry != NULL)
	{
		SharedTypmodTableEntry *entry;
		int32 shared_end =
			pg_atomic_read_u32(&CurrentSession->shared_typmod_registry->next_typmod);
		...
		for (; i < shared_end; i++)
		{
			...
			{
				TupleDescNode *node = palloc0(sizeof(TupleDescNode));
				TupleDesc tmp = (TupleDesc) dsa_get_address(CurrentSession->area,
															entry->shared_tupdesc);
				Assert(i == tmp->tdtypmod);
				node->type = T_TupleDescNode;
				node->natts = tmp->natts;
				node->tuple = CreateTupleDescCopy(tmp);
				transientTypeList = lappend(transientTypeList, node);
