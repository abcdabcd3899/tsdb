...
SerializeSnapshot(Snapshot snapshot, char *start_address)
{
	...
	memcpy(&serialized_snapshot.distribSnapshotWithLocalMapping,
	...
	/* Copy distributed snapshot, if present */
	if (serialized_snapshot.haveDistribSnapshot)
	{
		DistributedSnapshotWithLocalMapping	*dswlm;
		Size	offset;

		dswlm = (DistributedSnapshotWithLocalMapping *)&snapshot->distribSnapshotWithLocalMapping;
		offset = sizeof(SerializedSnapshotData) +
			(snapshot->xcnt + snapshot->subxcnt) * sizeof(TransactionId);

		/* Copy cache for distributed transaction and */
		memcpy((TransactionId *) (start_address + offset),
			   dswlm->inProgressMappedLocalXids,
			   dswlm->currentLocalXidsCount * sizeof(TransactionId));

		/* Copy distributed transations in progress */
		offset += dswlm->currentLocalXidsCount * sizeof(TransactionId);
		memcpy((DistributedTransactionId *) (start_address + offset),
			   dswlm->ds.inProgressXidArray,
			   dswlm->ds.count * sizeof(DistributedTransactionId));
	}
}
...
RestoreSnapshot(char *start_address)
{
	...
	/* Copy distributed snapshot */
	memcpy(&snapshot->distribSnapshotWithLocalMapping,
		   &serialized_snapshot.distribSnapshotWithLocalMapping,
		   sizeof(DistributedSnapshotWithLocalMapping));
	...
	if (serialized_snapshot.haveDistribSnapshot)
	{
		...
		/* copy mapping cache */
		size = dswlm->currentLocalXidsCount * sizeof(TransactionId);
		dswlm->inProgressMappedLocalXids =
			(TransactionId *) MemoryContextAlloc(TopTransactionContext, size);

		memcpy(dswlm->inProgressMappedLocalXids,
			   serialized_xids + serialized_snapshot.xcnt + serialized_snapshot.subxcnt,
			   size);

		/* copy distribued snapshot in progress */
		size = dswlm->ds.count * sizeof(DistributedTransactionId);
		dswlm->ds.inProgressXidArray =
			(DistributedTransactionId *) MemoryContextAlloc(TopTransactionContext, size);

		memcpy(dswlm->ds.inProgressXidArray,
			   serialized_xids + serialized_snapshot.xcnt +
			   serialized_snapshot.subxcnt + dswlm->currentLocalXidsCount,
			   size);
	}
