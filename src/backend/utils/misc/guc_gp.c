...
/* Disable partition validation */
bool		mx_partition_without_validation = false;
...
struct config_bool ConfigureNamesBool_gp[] =
{
...

	{
		...
		/* MatrixDB enables GDD by default */
		true, NULL, NULL
    },
	...
	{
		{"mx_partition_without_validation", PGC_USERSET, DEVELOPER_OPTIONS,
			gettext_noop("Enables alter partition table without validation."),
			NULL,
			GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE
		},
		&mx_partition_without_validation,
		false,
		NULL, NULL, NULL
	},
...
#ifdef MATRIXDB_VERSION
	{
		{"sortheap_sort_mem", PGC_USERSET, PRESET_OPTIONS,
			gettext_noop("Maximum memory can be used for quick sorting"),
			NULL,
			GUC_UNIT_KB
		},
		&sortheap_sort_mem,
...
struct config_real ConfigureNamesReal_gp[] =
{
	...
	{
		{"gp_motion_cost_startup", PGC_USERSET, QUERY_TUNING_COST,
			gettext_noop("Sets the planner's estimate of the cost of "
						 "startup a QE process"),
			gettext_noop("planner will multiply it by the number of QEs as the motion startup cost.")
		},
		&gp_motion_cost_startup,
		0, 0, DBL_MAX,
		NULL, NULL, NULL
	},
...
storageOptToString(const StdRdOptions *ao_opts)
{
	...
	if (ao_opts->parallel_workers >= 0)
	{
		appendStringInfo(&buf, "%s=%d,", SOPT_PARALLEL_WORKERS,
						 ao_opts->parallel_workers);
	}
