...
static MotionParallelShmem *motionParallelShmem = NULL;
...
ExecMotion(PlanState *pstate)
{
	...
	{
		if (motion->sender_parallel_workers && !node->attached)
		{
			MotionParallelEntry	*entry;
			MotionParallelKey	key;
			bool	found;	

			key.session_id = gp_session_id;
			key.command_id = gp_command_count;
			key.plan_node_id = node->ps.plan->plan_node_id;

			LWLockAcquire(MotionParallelContextLock, LW_EXCLUSIVE);

			entry = (MotionParallelEntry *) hash_search(motionParallelShmem->hash,
													   (void *) &key,
													   HASH_ENTER,
													   &found);

			/* I am the first one to setup the Parallel Context */
			if (!found)
			{
				ExecInitParallelContext_Leader((PlanState *)node);
				/* record the dsm segment for parallel context */
				entry->dsm_handler = dsm_segment_handle(node->seg);
				pg_atomic_init_u32(&entry->nattached, 0);
				pg_atomic_init_u32(&entry->ndetached, 0);
				entry->nparticipants = motion->sender_parallel_workers + 1;
				/* init the motion barrier */
				BarrierInit(&entry->barrier, entry->nparticipants);
			}
			else if (entry->dsm_handler)
			{
				ExecInitParallelContext_Worker((PlanState *) node,
											   entry->dsm_handler);
			}

			/* Set ParallelWorkerNumber correctly */
			ParallelWorkerNumber = pg_atomic_read_u32(&entry->nattached);

			/*
			 * Mark myself as attached and tell other workers.
			 */
			node->attached = true;
			pg_atomic_add_fetch_u32(&entry->nattached, 1);

			LWLockRelease(MotionParallelContextLock);
		}
...
ExecInitMotion(Motion *node, EState *estate, int eflags)
{
	...
	motionstate->seg = NULL;
	motionstate->area = NULL;
	/*
...
ExecEndMotion(MotionState *node)
{
	...
	if (node->mstype == MOTIONSTATE_SEND && motion->sender_parallel_workers)
	{
		MotionParallelKey	key;
		MotionParallelEntry	*entry;
		bool	found;	
		...
		/* Is it an re-entry ? */
		if (node->attached && node->detached)
		{
			/* do nothing */
		}
		else
		{
			...
			if (!node->attached)
			{
				...
				/*
				 * No other workers attached yet, initialize the parallel
				 * context for them although I will not use it.
				 */
				if (!found)
				{
					ExecInitParallelContext_Leader((PlanState *)node);
			...
			/*
			 * Wait all workers to reach here
			 *
			 * when multiple workers execute a slice with the
			 * parallel plan like:
			 *
			 * 	Redistribute Motion (parallel_workers = 2)
			 * 		--> Hash Aggregate
			 * 			--> parallel scan (parallel_workers = 2)
			 *
			 * 	In case, worker#1 have finished the slice and worker#2
			 * 	have not even started executing plan. worker#1 has two
			 * 	options:
			 *
			 * 	1. wait worker#2 finish the work () and then destory the
			 * 	parallel context
			 *
			 * 	2. destory the parallel context and tell worker#2 to
			 * 	finish early.
			 *
			 * 	The option 2 unfortunately doesn't work for bellow case:
			 *
			 * 	Redistribute Motion (parallel_workers = 2)
			 * 		--> Hash Aggregate
			 * 			--> Redistribute Motion (parallel_workers = 2)
			 *
			 * 	if worker#2 finish earlier, it will lose the data that its
			 * 	children sent.
			 */
			if (BarrierArriveAndWait(&entry->barrier,
									 WAIT_EVENT_MOTION_PARALLEL_TEARDOWN))
			{
				int count = 0;
				/* Mark myself detached, do this before entry is released */
				node->detached = true;
				pg_atomic_add_fetch_u32(&entry->ndetached, 1);

				/*
				 * I am elected as the winner to free the hash entry, other
				 * workers may still being waken up from barrier, so make sure
				 * other workers has been detached successfully before the
				 * entry->barrier is freed.
				 */
				while (pg_atomic_read_u32(&entry->ndetached) != entry->nparticipants)
				{
					CHECK_FOR_INTERRUPTS();
					pg_usleep(1000 /* 1 ms */);

					/* report FATAL after 30 second */
					if (count++ > 1000 * 30)
						elog(FATAL, "parallel workers cannot detach in 30 seconds");
				}

				LWLockAcquire(MotionParallelContextLock, LW_EXCLUSIVE);
				hash_search(motionParallelShmem->hash,
							(void *) &key, HASH_REMOVE, &found);
				LWLockRelease(MotionParallelContextLock);
			}
			else
			{
				/* Mark myself detached. */
				node->detached = true;
				pg_atomic_add_fetch_u32(&entry->ndetached, 1);
			}

			/*
			 * Detach myself from the parallel context
			 *
			 * NOTICE: if I am the last worker has attached to the
			 * dsm, the dsm_detach() will release all the dsm, so
			 * we need to let other workers know the parallel
			...
			if (node->seg)
			{
				dsm_detach(node->seg);
				node->seg = NULL;
				dsa_detach(node->area);	
				node->area = NULL;
			}
		}

		/* reset ParallelWorkerNumber */
...
MotionParallelShmemInit(void)
{
	...
	motionParallelShmem =
		(MotionParallelShmem *) ShmemInitStruct("Motion Parallel Shmem",
												sizeof(MotionParallelShmem),
												&foundPtr);

	if (motionParallelShmem == NULL)
		ereport(ERROR,
				(errcode(ERRCODE_OUT_OF_MEMORY),
				 (errmsg("not enough shared memory for motion parallel shmem"))));

	MemSet(&info, 0, sizeof(info));
	info.keysize = sizeof(MotionParallelKey); 
	info.entrysize = sizeof(MotionParallelEntry);

	motionParallelShmem->hash = ShmemInitHash("Motion Parallel Hash",
											  16,
											  MaxConnections / 2,
											  &info,
