...
ExecSerializePlan(Plan *plan, EState *estate)
{
	...
	pstmt->subplan_sliceIds = palloc(list_length(pstmt->subplans) * sizeof(int ));
 ...
 *
 * ExecInitParallelContext_Leader is used to initialize
 * the parallel context by motion leader.
 *
 * ExecInitParallelContext_Worker is used to initialize
 * the parallel context by motion leader.
 */
void
ExecInitParallelContext_Leader(PlanState *pstate)
{
	ExecParallelEstimateContext e;
	ExecParallelInitializeDSMContext d;
	ParallelContext pcxt;
	MotionState *ms = castNode(MotionState, pstate);
	Motion		*motion = (Motion *) pstate->plan;
	Size		dsa_minsize = dsa_minimum_size();
	char		*area_space;

	Assert(IsA(pstate, MotionState));

	pcxt.nworkers = motion->sender_parallel_workers + 1;

	/* step1: estimate the shmem size we need */
	shm_toc_initialize_estimator(&pcxt.estimator);
	e.pcxt = &pcxt;
	e.nnodes = 0;
	ExecParallelEstimate(ms->ps.lefttree, &e);
	...
	/* 
	 * step2: create the shmem, this is a short version
	 * of InitializeParallelDSM
	 */
	InitializeParallelDSM_short(&pcxt);


	area_space = shm_toc_allocate(pcxt.toc, dsa_minsize);
	shm_toc_insert(pcxt.toc, PARALLEL_KEY_DSA, area_space);
	ms->area = dsa_create_in_place(area_space, dsa_minsize,
								   LWTRANCHE_PARALLEL_QUERY_DSA,
								   pcxt.seg);

	/* step3: init the parallel context for nodes */
	d.pcxt = &pcxt;
	d.instrumentation = NULL;
	d.nnodes = 0;

	/* Special executor initialization steps for parallel workers */
	pstate->state->es_query_dsa = ms->area;

	ExecParallelInitializeDSM(ms->ps.lefttree, &d);

	ms->seg = pcxt.seg;
}

void
ExecInitParallelContext_Worker(PlanState *pstate, dsm_handle handle)
{
	ParallelWorkerContext pwcxt;
	MotionState *ms = castNode(MotionState, pstate);
	dsm_segment *seg;
	...
	/*
	 * use INTERCONNECT errorcode here so master can treat this error
	 * as lower priority error
	 */
	if (seg == NULL)
		ereport(ERROR,
				(errcode(ERRCODE_GP_INTERCONNECTION_ERROR),
				 errmsg("could not attach to DSM segment, the leader might be die")));

	toc = shm_toc_attach_parallel(seg);

	/* Attach to the dynamic shared memory area. */
	area_space = shm_toc_lookup(toc, PARALLEL_KEY_DSA, false);
	ms->area = dsa_attach_in_place(area_space, seg);
	...
	ExecParallelInitializeWorker(pstate->lefttree, &pwcxt);
