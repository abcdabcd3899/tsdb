						...
						PartitionTupleRouting *proute)
{
	...
	for (i = 0; i < proute->num_partitions; i++)
	{
		...
		if (!RelationIsAppendOptimized(resultRelInfo->ri_RelationDesc))
			table_event_notify(resultRelInfo->ri_RelationDesc, mtstate->operation, NULL);
#endif /* MATRIXDB_VERSION */
	...
	if (join_prune_paramids)
	{
		...
		foreach (lc, join_prune_paramids)
		{
			...
			if (psstate == NULL)
			{
				 ...
				 *
				 * psstate will be NULL in parallel mode which uses a GATHER node
				 * because the param can not be passed through a GATHER node,
				 * the partition selector is not actually work in this case, it
				 * has no harm on result.
				 */
				if (!IsParallelWorker())
