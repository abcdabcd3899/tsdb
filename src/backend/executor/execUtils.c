...
FillSliceGangInfo(ExecSlice *slice, PlanSlice *ps)
{
	...
	switch (slice->gangType)
	{
		...
		case GANGTYPE_PRIMARY_READER:
			...
			if (ps->parallel_workers > 0)
			{
				int i;
				List *extra = NIL;
				for (i = 0; i < ps->parallel_workers; i++)
				{
					extra = list_concat(extra, list_copy(slice->segments));
				}
				slice->segments = list_concat(slice->segments, extra);
			}
