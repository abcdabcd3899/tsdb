...
ExecInterpExpr(ExprState *state, ExprContext *econtext, bool *isnull)
{
	...
	strictfail:
		...
		EEO_CASE(EEOP_ROWIDEXPR)
		{
			...
			if (op->d.rowidexpr.rowcounter == -1)
			{
				if (ParallelWorkerNumber >= 0)
					op->d.rowidexpr.rowcounter =
						(((int64) GpIdentity.dbid) << 42) |
						(((int64) ParallelWorkerNumber) << 56) ;
				else
					op->d.rowidexpr.rowcounter = ((int64) GpIdentity.dbid) << 48;
			}

			rowcounter = ++op->d.rowidexpr.rowcounter;
