...
MultiExecBitmapIndexScan(BitmapIndexScanState *node)
{
	...
	/* shared bitmap scan is only enabled for btam which is a TIDBitmap */
	if (((BitmapIndexScan *) node->ss.ps.plan)->isshared)
		node->biss_result =
			(Node *) tbm_create(work_mem * 1024L, node->ss.ps.state->es_query_dsa);
