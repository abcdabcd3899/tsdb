...
TupleTableSlot *
sort_advance_aggregate(AggState *aggstate, TupleTableSlot *slot)
{
	TupleTableSlot	*firstGroupSlot = aggstate->ss.ss_ScanTupleSlot;
	TupleTableSlot	*result = NULL;
	ExprContext		*tmpcontext = aggstate->tmpcontext;
	ExprContext		*econtext = aggstate->ss.ps.ps_ExprContext;
	Agg				*agg = aggstate->phase->aggnode;
	...
	/* Set outertuple of tmpcontext for group boundary check */
...
retry:
	...
	{
		/*
		 * Reset the pertrans context, we are starting the
		 * aggregate of new group.
		 */
		ResetExprContext(econtext);

		/* No groupingsets, so only reset the first group */
		ReScanExprContext(aggstate->aggcontexts[0]);

		initialize_aggregates(aggstate, aggstate->pergroups, 1);
		/*
		 * Store the first group tuple so we can tell the group
		 * boundary
		 */
		aggstate->grp_firstTuple = ExecCopySlotHeapTuple(slot);
		ExecForceStoreHeapTuple(aggstate->grp_firstTuple,
								firstGroupSlot, true);

		econtext->ecxt_outertuple = firstGroupSlot;

		advance_aggregates(aggstate);

		/* We have not enough tuple to check group boundary now */
		return result;
	}

	/* Set inner tuple so we can check the group boundary */
	tmpcontext->ecxt_innertuple = firstGroupSlot;

	if (aggstate->aggstrategy == AGG_PLAIN ||
		ExecQual(aggstate->phase->eqfunctions[agg->numCols -1], tmpcontext))
	{
		/*
		 * The new tuple is still belong to the current group,
		 * we can continue to do the transform
		 */
		advance_aggregates(aggstate);
		/* Doesn't reach a group boundary */
		return NULL;
	}
	else
	{
		...
		/* 
		 * Materialize the result before retry, retry code will reset
		 * the expr context.
		 */
