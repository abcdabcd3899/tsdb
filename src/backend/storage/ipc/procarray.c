...
copyLocalSnapshot(Snapshot snapshot)
{
	...
	/*
	 * snapshot->subxcnt = -1;
	 * will cause a assert failure in parallel mode
	 */
	snapshot->subxcnt = 0;
...
readerFillLocalSnapshot(Snapshot snapshot, DtxContext distributedTransactionContext)
{
	...
	for (;;)
	{
			...
			SharedLocalSnapshotSlot->ready)
		{
			...
			/*
			 * Also assign the MyPgXact->xmin for the reader, if the reader
			 * start the parallel workers, the workers will check the
			 * xmin when restore the snapshot.
			 */
