...
#ifdef MATRIXDB_VERSION
	if (pstate->p_is_insert && RelationIsMars(pstate->p_target_relation))
	{
		/* MARS does not support concurrent INSERT at the moment */
		LockRelationOid(RelationGetRelid(pstate->p_target_relation),
						ExclusiveLock);
	}
#endif /* MATRIXDB_VERSION */
