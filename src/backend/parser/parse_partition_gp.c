						...
						bool *hasImplicitRangeBounds)
{
	...
	else
	{
		...
		partcolname = deparse_expression((Node *) linitial(partexprs),
										 deparse_context_for(RelationGetRelationName(parentrel),
				   ...
				   List *parentattenc, bool forvalidationonly)
{
	...
	if (GpPolicyIsEntry(parentrel->rd_cdbpolicy))
	{
		/* MASTERONLY table do not support PARTITION. */
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("%s is DISTRIBUTED MASTERONLY, cannot set PARTITION.",
