			 ...
			 char *completionTag)
{
	...
	if (Gp_role == GP_ROLE_DISPATCH)
	{
		...
		/*
		 * Skip FDW auto stats, auto_stats will execute ANALYZE if some conditions 
		 * are satisfied. Most of FDWs are grab from upstream, they do not support 
		 * ANALYZE.
		 * Open the relation with NoLock, since we only need to know if it is a FDW.
		 */
