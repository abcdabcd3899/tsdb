...
exec_simple_query(const char *query_string)
{
	...
	char		msec_str[32];
	bool		fast_direct_dispatch = false;
	 ...
	 */
	if (TrySingleDirectDispatchFashPath(parsetree_list, query_string))
	{
		/* don't need to run the query in the QD */
		fast_direct_dispatch = true;
		parsetree_list = NIL;
	}
	 ...
	 */
	if (!parsetree_list && !fast_direct_dispatch)
		NullCommand(dest);
