...
#if defined(MATRIXDB_VERSION) && BLCKSZ < 32768
	if (relkind == RELKIND_AOSEGMENTS)
		/*
		 * MatrixDB: we have changed blocksize to 8KB in mxdb, so an ao/aoco
		 * table with 1600 int columns will have a too wide vpinfo of the
		 * aoseg/aocsseg, which can not be stored at all.  We have to enable
		 * toasting for it.
		 *
		 * The in-place compression might not be enough to store the wide
		 * vpinfo, we also need to assign a toast rel for the aoseg/aocsseg.
		 */
		AlterTableCreateToastTable(aoauxiliary_relid,
								   (Datum) 0, AccessExclusiveLock);
