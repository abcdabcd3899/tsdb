...
#ifdef MATRIXDB_VERSION
	/*
	 * For mars option, it is saved in mx_mars catalog
	 */
	if (accessmtd != InvalidOid && accessmtd != HEAP_TABLE_AM_OID)
	{
		Oid marsOid = get_table_am_oid("mars", true);

		if (accessmtd == marsOid)
		{
			InsertMARSEntryWithValidation(relid, reloptions, tupdesc);
		}
...
#ifdef MATRIXDB_VERSION
	is_plugin_rel = !(RelationIsAppendOptimized(rel)
		|| RelationIsHeap(rel)
...
#ifdef MATRIXDB_VERSION
	else if (is_plugin_rel)
	{
		/*
		 * must open with relation_open() instead of table_open(), the latter
		 * raises exceptions on non-table relkind's, however we want to
		 * silently ignore them.
		 */
		rel = relation_open(relid, NoLock);
		table_event_notify(rel, CMD_UTILITY, "DropTable");
		relation_close(rel, NoLock);
	}
#endif /* MATRIXDB_VERSION */
