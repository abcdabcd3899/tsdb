 ...
 * ReadLastSequences
 *
 * Read the last_sequence attribute from gp_fastsequence by obiid and the
 * objmod range, return all the matched last_sequences.
 *
 * The results are saved in the argument "results", the array size must be
 * >= (maxObjmod - minObjmod + 1).
 *
 * Return the count of the results.
 */
int64
ReadLastSequences(Oid objid,
				  int64 minObjmod,
				  int64 maxObjmod,
				  FormData_gp_fastsequence *results)
{
	Relation	gp_fastsequence_rel;
	...
	/*
	 * SELECT * FROM gp_fastsequence
	 * WHERE objid = :1 AND objmod >= :2 AND objmod <= :3
	 */
	ScanKeyInit(&scankey[0],
				Anum_gp_fastsequence_objid,
				BTEqualStrategyNumber, F_OIDEQ,
	...
	ScanKeyInit(&scankey[1],
				...
				Int64GetDatum(minObjmod));
	ScanKeyInit(&scankey[2],
				Anum_gp_fastsequence_objmod,
				BTLessEqualStrategyNumber, F_INT8LE,
				Int64GetDatum(maxObjmod));
	...
	while (HeapTupleIsValid((tuple = systable_getnext(scan))))
	{
		...
		objmod = heap_getattr(tuple, Anum_gp_fastsequence_objmod,
							  tupleDesc, &isNull);
		Assert(!isNull);
		...
		results[nResults].objid = objid;
		results[nResults].objmod = objmod;
		results[nResults].last_sequence = lastSequence;
		nResults++;
	}
