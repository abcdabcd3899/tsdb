...
static void marsPkgOptionValidation(relopt_value *option);
#endif
/*
				  ...
				  ArrayBuildState *astate)
{
	...
	else if (pg_strcasecmp(SOPT_PARALLEL_WORKERS, name) == 0)
	{
		if (!parse_int(value, &intval, 0 /* unit flags */ ,
					   NULL /* hint message */ ))
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("invalid integer value \"%s\" for storage option \"%s\"",
							value, name)));
		appendStringInfo(&buf, "%s=%d", SOPT_PARALLEL_WORKERS, intval);
	}
	else
	{
		/*
...
transformAOStdRdOptions(StdRdOptions *opts, Datum withOpts)
{
	...
	if (DatumGetPointer(withOpts) != NULL)
	{
		...
		for (i = 0; i < nWithOpts; ++i)
		{
			...
			soptLen = strlen(SOPT_PARALLEL_WORKERS);
			if (withLen > soptLen &&
				pg_strncasecmp(strval, SOPT_PARALLEL_WORKERS, soptLen) == 0)
			{
				foundComplevel = true;
				d = CStringGetTextDatum(psprintf("%s=%d",
												 SOPT_PARALLEL_WORKERS,
							...
							int num_options, relopt_kind kind, bool validate)
{
	...
	relopt_value *parallel_workers_opt;
...
#ifdef MATRIXDB_VERSION
	{
		/* Simple grammar option set */
		relopt_value *tag_op = get_option_set(options, num_options, MX_MARS_TAG_KEY);
		relopt_value *time_op = get_option_set(options, num_options, MX_MARS_TIME_KEY);
		relopt_value *time_bucket_op = get_option_set(options, num_options, MX_MARS_TIME_BUCKET);
		/* Accurate grammar option set */
		relopt_value *group_ = get_option_set(options, num_options, MX_MARS_GROUP_COLUMN);
		relopt_value *group_param_ =
			get_option_set(options, num_options, MX_MARS_GROUP_PARAMETER_COLUMN);
		relopt_value *global_order_ =
			get_option_set(options, num_options, MX_MARS_GLOBAL_ORDER_COLUMN);
		relopt_value *local_order_ =
			get_option_set(options, num_options, MX_MARS_LOCAL_ORDER_COLUMN);

		/* validate accurate grammar options if no simple grammar option exist */
		if (!tag_op && !time_op && !time_bucket_op)
		{
			marsPkgOptionValidation(group_);
			marsPkgOptionValidation(group_param_);
			marsPkgOptionValidation(global_order_);
			marsPkgOptionValidation(local_order_);
		}
		else
		{
			if (group_ || group_param_ || global_order_ || local_order_)
			{
				ereport(ERROR,
						(errcode(ERRCODE_WITH_CHECK_OPTION_VIOLATION),
						 errmsg("MARS does not support mixed use simple grammar options with accurate grammar options"),
						 errdetail("Simple grammar: %s, %s, %s; Accurate grammar: %s, "
								   "%s, %s, %s",
								   MX_MARS_TAG_KEY,
								   MX_MARS_TIME_KEY,
								   MX_MARS_TIME_BUCKET,
			...
			if (time_bucket_op != NULL && time_op == NULL)
				ereport(ERROR,
						(errcode(ERRCODE_WITH_CHECK_OPTION_VIOLATION),
						 errmsg("%s option rely on %s option to define time column",
								MX_MARS_TIME_BUCKET,
								MX_MARS_TIME_KEY)));
		}
	}
	...
	/* parallel_workers */
	parallel_workers_opt = get_option_set(options, num_options, SOPT_PARALLEL_WORKERS);
	if (parallel_workers_opt != NULL)
		result->parallel_workers = parallel_workers_opt->values.int_val;
							...
							int numrelopts, relopt_kind kind, bool validate)
{
		...
		KIND_IS_APPENDOPTIMIZED(kind))
	{
		...
		if (!(get_option_set(options, numrelopts, SOPT_PARALLEL_WORKERS)))
			result->parallel_workers = ao_storage_opts.parallel_workers;
	}
...
void marsPkgOptionValidation(relopt_value *option)
{
	Oid			typinput,
	            typioparam;
	...
	{
		Datum pI;
		char  *param = option->values.string_val;

		/*
		 * In parser stage, only check if it is a text array.
		 * Further constraint check process in mars set file node callback
		 */
		PG_TRY();
		{
			getTypeInputInfo(TEXTARRAYOID, &typinput, &typioparam);
			pI = OidInputFunctionCall(typinput, param, typioparam, -1);
		}
		PG_CATCH();
		{
			ereport(ERROR,
			        (errcode(ERRCODE_FDW_INVALID_STRING_FORMAT),
				        errmsg("MARS option only accept text array.")));
			PG_RE_THROW();
		}
		PG_END_TRY();

		pfree(DatumGetArrayTypeP(pI));
	}
}
...
fillin_encoding(List *aocoColumnEncoding)
{
	...
	foreach(lc, aocoColumnEncoding)
	{
		...
		else if (pg_strcasecmp("compresslevel", el->defname) == 0)
		{
			...
			foundBlockSize = true;
		else if (pg_strcasecmp(SOPT_PARALLEL_WORKERS, el->defname) == 0)
			parallel_workers = defGetString(el);
	}
	...
	if (parallel_workers)
	{
		el = makeDefElem(SOPT_PARALLEL_WORKERS,
						 (Node *) makeInteger(ao_opts->parallel_workers), -1);
		retList = lappend(retList, el);
	}
	return retList;
}
