...
TransactionIdIsCurrentTransactionId(TransactionId xid)
{
	 ...
	 *
	 * CDB: parallel worker may be launched by a reader segment and the reader
	 * segment's transation state is also valid, it will check its writer's
	 * transaction state instead. So let the parallel worker also use the writer
	 * 's transaction state to check whether it is the current transaction.
	 */
	if (false && nParallelCurrentXids > 0)
	{
		int			low,
					high;
