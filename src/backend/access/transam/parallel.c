...
#define PARALLEL_KEY_DTXCONTEXT 			UINT64CONST(0xFFFFFFFFFFFF000E)

#ifndef MATRIXDB_VERSION
/* 
 * In Matrixdb, FixedParallelState and MyFixedParallelState are moved to
 * parallel.h, be careful when merging codes
 */
...
InitializeParallelDSM(ParallelContext *pcxt)
{
	...
	fps->numsegmentsFromQD = numsegmentsFromQD;
	...
	if (pcxt->nworkers > 0)
	{
		...
		*(dsm_handle *) session_dsm_handle_space = session_dsm_handle;
		...
		/* Serialize DTX context */
		dtxcontextspace = shm_toc_allocate(pcxt->toc, dtxcontextlen);
		memcpy(dtxcontextspace, serializedDtxContextInfo, dtxcontextlen);
		shm_toc_insert(pcxt->toc, PARALLEL_KEY_DTXCONTEXT, dtxcontextspace);
...
ParallelWorkerMain(Datum main_arg)
{
	...
	/* CDB: parallel workers are never be the writer and the dispatcher */
	Gp_role = GP_ROLE_EXECUTE;
	gp_session_id = fps->session_id;
	MyProc->mppSessionId = fps->session_id;
	numsegmentsFromQD = fps->numsegmentsFromQD;
	...
	dtxcontextspace = shm_toc_lookup(toc, PARALLEL_KEY_DTXCONTEXT, false);
	/*
	 * The deserialization logic will also reset the dtx context, however it
	 * will try to reuse dtxcontext.distributedSnapshot.inProgressXidArray, so
	 * we must set it to NULL manually before that.  Do a full memset just for
	 * sure.
	 */
...
shm_toc *
shm_toc_attach_parallel(dsm_segment *seg)
{
	return shm_toc_attach(PARALLEL_MAGIC, dsm_segment_address(seg));
}

/*
 * In postgres, GATHER node initialize the parallel context and
 * launch the parallel workers with the parallel context.
 *
 * In Matrix DB, for some plans, GATHER node is not generated,
 * a MOTION node is instead added on the top of the partial
 * paths. The QD will launch all the parallel workers as QEs
 * and QEs of motion senders of the same MOTION will race to
 * become the leader to initialize the parallel context.
 *
 * InitializeParallelDSM_short is the short version of
 * InitializeParallelDSM, it is used by motion leader to
 * initialize parallel context, QD has synced most of the parallel
 * context to QEs, so it is much simpler.
 *
 */
void
InitializeParallelDSM_short(ParallelContext *pcxt)
{
	MemoryContext oldcontext;
	int segsize;
