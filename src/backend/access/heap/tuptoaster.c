...
toast_delete(Relation rel, HeapTuple oldtup, bool is_speculative)
{
	...
	Assert(rel->rd_rel->relkind == RELKIND_RELATION ||
		   ...
		   /* MatrixDB: refer to CreateAOAuxiliaryTable() for details. */
...
/* Wrapper function to call toast_save_datum*/
Datum
sortheap_toast_save_datum(Relation rel, Datum value)
{
	return toast_save_datum(rel, value, NULL, false, 0);
}
#endif
