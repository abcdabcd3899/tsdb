			  ...
			  ScanDirection dir, bool *continuescan)
{
	...
	int			keysz;
	int			ikey;
	int			odin_max_trusted_attno USED_FOR_ODIN_ONLY = -1;
	int			odin_first_unequal_attno USED_FOR_ODIN_ONLY = -1;
	ScanKey		key;

	ODIN_FEATURE_BEGIN(INDEXSCAN, RelationGetRelid(scan->indexRelation));
	ODIN_FEATURE_OTHERWISE(INDEXSCAN);
	...
	*continuescan = true;		/* default assumption */
	...
	ODIN_FEATURE_BEGIN(INDEXSCAN, RelationGetRelid(scan->indexRelation));
	/* In ODIN we scan the heap instead of the index */
	tupdesc = RelationGetDescr(odin_get_heap_relation(scan));

	/*
	 * TODO: to access the heap slot we need both "scan" and "so", they are
	 * available in this function, however missing in _bt_check_rowcompare().
	 * Notice that the argument "tuple", the index tuple, is meaningless for
	 * ODIN, we pass "scan" via this argument, this is very dirty, how could
	 * we avoid it?
	 */
	tuple = (IndexTuple) scan;
	...
	for (key = so->keyData, ikey = 0; ikey < keysz; key++, ikey++)
	{
		...
		ODIN_FEATURE_BEGIN(BITMAPSCAN, RelationGetRelid(scan->indexRelation));
		/*
		 * For ODIN bitmapscan we can only trustthe first key, for example,
		 * suppose the tuples in a blocks are:
		 *
		 *     {1,10}  {1,30}  {4,10}
		 *
		 * the index record is [{1,10}, {4,10}], when searching for >= {1,30}
		 * we can not know the 2nd key 30 matches or not.
		 *
		 * however when searching for < {1,6}, the 10 can be trusted, so here
		 * are a list of patterns that can be trusted:
		 * - < {1,10}: mismatch
		 * - > {4,10}: mismatch
		 * - <= {1,10}: match
		 * - >= {4,10}: match
		 * - >= {1,x} where x>=10: match
		 * - <= {4,x where x<=10}: match
		 *
		 * TODO: to be continued
		 *
		 * furthermore, if the first key has the same value in both edges, then
		 * we could trust the second one.  In fact we could trust the keys
		 * until the first one which differs in the two edges. for example,
		 * when the tuples are:
		 *
		 *     {1,10}  {1,20}  {1,30}
		 *
		 * we can also trust below patterns:
		 * - {1,15} <= sk <= {1,25}: match
		 */
		if (key->sk_attno > 1)
			break;

		if (key->sk_attno <= odin_max_trusted_attno)
		{
			/* trust the current sk_attno */
		}
		else if (odin_first_unequal_attno >= 1)
		{
			/* already know the first unequal attno */
			Assert(odin_max_trusted_attno == odin_first_unequal_attno);
			break;
		}
		else
		{
			...
			indnkeyatts = IndexRelationGetNumberOfKeyAttributes(scan->indexRelation);
			datum1 = index_getattr(tuple, sk_attno,
								   tupdesc, &isnull1);
			datum2 = index_getattr(tuple, sk_attno + indnkeyatts,
								   tupdesc, &isnull2);

			Assert(!isnull1);
			Assert(!isnull2);

			/* the current sk_attno can be trusted */
			odin_max_trusted_attno = sk_attno;

			if (!datumIsEqual(datum1, datum2,
							  tupdesc->attrs[sk_attno].attbyval,
							  tupdesc->attrs[sk_attno].attlen))
				odin_first_unequal_attno = sk_attno;
		}

		ODIN_FEATURE_END(BITMAPSCAN);
		...
		switch (key->sk_strategy)
		{
			...
			case BTGreaterEqualStrategyNumber:
				/* (end edge) >= key */
				sk_attno += IndexRelationGetNumberOfKeyAttributes(scan->indexRelation);
				break;

			case BTEqualStrategyNumber:
				/* (start edge) <= key <= (end edge) */
				/* TODO: we need to construct the cmp functions */
				elog(ERROR, "ODIN: TODO: support BTEqualStrategyNumber");
				break;

			default:
				elog(ERROR, "ODIN: unsupported sk_strategy: %d",
					 key->sk_strategy);
				break;
		}
		...
		ODIN_FEATURE_OTHERWISE(BITMAPSCAN);
...
{
	/* FIXME: how to avoid this trick? */
	IndexScanDesc scan USED_FOR_ODIN_ONLY = (IndexScanDesc) tuple;
	BTScanOpaque so USED_FOR_ODIN_ONLY = (BTScanOpaque) scan->opaque;
	ScanKey		subkey = (ScanKey) DatumGetPointer(skey->sk_argument);
	int32		cmpresult = 0;
	bool		result;
