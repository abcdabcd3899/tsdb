...
#ifdef ENABLE_ODIN
Oid			odin_indexoid = InvalidOid;
unsigned int odin_features = 0;
#endif  /* ENABLE_ODIN */
...
btgetbitmap(IndexScanDesc scan, Node *n)
{
	...
	do
	{
		...
		if (_bt_first(scan, ForwardScanDirection))
		{
			...
			tbm_add_page(tbm, ItemPointerGetBlockNumber(heapTid));
...
_bt_parallel_seize(IndexScanDesc scan, BlockNumber *pageno)
{
	...
	*pageno = P_NONE;
	...
	{
		/*
		 * We have seized the cv, it is our responsibility to find out the next
		 * page to scan.  Remember this status so we can release the cv later
		 * when we know the next page.
		 */
		Assert(!so->odin_advancing);
		so->odin_advancing = true;
	}
