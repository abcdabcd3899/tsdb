			...
			OffsetNumber offnum)
{
	...
	for (int i = 1; i <= ncmpkey; i++)
	{
		...
		ODIN_FEATURE_BEGIN(ALL, RelationGetRelid(rel));

		int			sk_attno = scankey->sk_attno;

		/*
		 * For non-meta tuples we need to decide the edge to compare, such kind
		 * of tuples only exist on leaf pages, and must not contain the
		 * INDEX_ALT_TID_MASK bit, see BTreeTupleGetNAtts() for more
		 * information.
		 */
		if (P_ISLEAF(opaque) &&
			!(itup->t_info & INDEX_ALT_TID_MASK))
		{
			...
			switch (scankey->sk_strategy)
			{
				case BTEqualStrategyNumber:
					/* (end edge) >= scankey */
					sk_attno += rel->rd_index->indnkeyatts;
					break;
		...
		datum = index_getattr(itup, sk_attno, itupdesc, &isNull);

		ODIN_FEATURE_OTHERWISE(ALL);

		datum = index_getattr(itup, scankey->sk_attno, itupdesc, &isNull);

		ODIN_FEATURE_END(ALL);
...
_bt_first(IndexScanDesc scan, ScanDirection dir)
{
	...
	if (scan->parallel_scan != NULL)
	{
		...
		else if (blkno != InvalidBlockNumber)
		{
			...
			ODIN_FEATURE_BEGIN(INDEXSCAN, RelationGetRelid(scan->indexRelation));

			/*
			 * The btgettuple() distinguishes the first scan by checking the
			 * currPos, so although we don't use it in odin, we have to set it
			 * to a valid pos, use the meta page for now.
			 */
			so->currPos.currPage = 0;
			so->currPos.buf = ReadBuffer(scan->indexRelation,
										 so->currPos.currPage);
			LockBuffer(so->currPos.buf, BT_READ);
			return true;

			ODIN_FEATURE_END(INDEXSCAN);
	...
	for (i = 0; i < keysCount; i++)
	{
		...
		else
		{
			...
			ODIN_FEATURE_BEGIN(ALL, RelationGetRelid(scan->indexRelation));
			inskey.scankeys[i].sk_strategy = cur->sk_strategy;
			ODIN_FEATURE_END(ALL);
		}
	}
	...
	BlockNumber	h_blkno = ItemPointerGetBlockNumber(&itup->t_tid);
	OffsetNumber h_off = odin_binsearch_heap(scan, &itup->t_tid);

	return odin_first_match(scan, dir, h_blkno, h_off);
...
_bt_next(IndexScanDesc scan, ScanDirection dir)
{
	...
	/*
	 * Move to next matching heap tid without accessing the index.
	 *
	 * TODO: we assume that dead tuples are impossible in ODIN.
	 */

	int			indnatts = RelationGetNumberOfAttributes(scan->heapRelation);
	BlockNumber	h_blkno = ItemPointerGetBlockNumber(&scan->xs_heaptid);
	OffsetNumber h_off = ItemPointerGetOffsetNumber(&scan->xs_heaptid);
	OffsetNumber h_minoff;
	OffsetNumber h_maxoff;
	bool		found = false;

	odin_get_heap_nblocks(scan);
	odin_get_heap_block_ntuples(scan, h_blkno, &h_minoff, &h_maxoff);
	...
	while (true)
	{
		...
		if (ScanDirectionIsForward(dir))
		{
			if (h_off == h_maxoff)
			{
				 ...
				 *
				 * For parallel scan, we need to find the page from the shared
				 * memory, but only if we have not seized already.
				 */
				if (scan->parallel_scan && !so->odin_advancing)
				{
					if (!_bt_parallel_seize(scan, &h_blkno))
						/* No more page to scan */
						break;
				}
				/*
				 * Otherwise, it is non-parallel scan, or we have already
				 * seized the cv, just move to the next page sequentialy.
				 */
				else
				{
				...
				Assert(BlockNumberIsValid(h_blkno));
				Assert(h_blkno >= so->odin_firstBlock);
				Assert(h_blkno <= so->odin_lastBlock);

				odin_get_heap_block_ntuples(scan, h_blkno, &h_minoff, &h_maxoff);
				h_off = h_minoff;
			}
			else
				h_off = OffsetNumberNext(h_off);
		}
		else
		{
			if (h_off == h_minoff)
			{
				/*
				 * It is time to move to the previous page.
				 *
				 * For parallel scan, we need to find the page from the shared
				 * memory, but only if we have not seized already.
				 */
				...
				/*
				 * Otherwise, it is non-parallel scan, or we have already
				 * seized the cv, just move to the previous page sequentialy.
				 */
				else
				{
					if (h_blkno == so->odin_firstBlock)
						/* No more block to scan */
						break;
				...
				h_off = OffsetNumberPrev(h_off);
		}

		if (!odin_get_heap_tuple(scan, h_blkno, h_off))
			break;

		found = _bt_checkkeys(scan, NULL /* tuple */,
							  indnatts, dir, &continuescan);
		if (found || !continuescan)
			break;
	}
	...
	if (found)
	{
		...
		/*
		 * If we are holding the cv in parallel scan, it is time to release the
		 * cv, and set the next page in shared memory for other parallel
		 * workers to continue.
		 */
		if (scan->parallel_scan && so->odin_advancing)
...
_bt_parallel_readpage(IndexScanDesc scan, BlockNumber blkno, ScanDirection dir)
{
	...
	/*
	 * In odin mode, the block is on the heap, we just need to seek the first
	 * tuple on it.
	 */

	BlockNumber	h_blkno = blkno;	/* blkno is the heap blkno in odin mode */
	OffsetNumber h_off;
	OffsetNumber h_minoff;
	OffsetNumber h_maxoff;

	/* TODO: validate the blkno? */
	odin_get_heap_nblocks(scan);
	odin_get_heap_block_ntuples(scan, h_blkno, &h_minoff, &h_maxoff);
...
_bt_endpoint(IndexScanDesc scan, ScanDirection dir)
{
	...
	ItemId		iid = PageGetItemId(page, start);
	IndexTuple	itup = (IndexTuple) PageGetItem(page, iid);
	BlockNumber	h_blkno = ItemPointerGetBlockNumber(&itup->t_tid);
	OffsetNumber h_minoff;
	OffsetNumber h_maxoff;
