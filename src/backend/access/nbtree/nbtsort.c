...
typedef struct BTBuildState
{
	...
	/*
	 * below attributes are only used by odin.
	 */
	ItemPointerData odin_tupleid;
	bool		odin_tuplelive;
	int			odin_count;
	Datum		odin_values[INDEX_MAX_KEYS];
	bool		odin_isnull[INDEX_MAX_KEYS];
} BTBuildState;
...
static void odin_build_callback(Relation index,
								ItemPointer tupleId,
								Datum *values,
								bool *isnull,
								bool tupleIsAlive,
...
btbuild(Relation heap, Relation index, IndexInfo *indexInfo)
{
	...
	ODIN_FEATURE_BEGIN(ALL, RelationGetRelid(index));
	ItemPointerSetInvalid(&buildstate.odin_tupleid);
					...
					IndexInfo *indexInfo)
{
	...
	if (!buildstate->btleader)
	{
		...
		reltuples = table_index_build_scan(heap, index, indexInfo, true, true,
										   odin_build_callback, (void *) buildstate,
										   NULL);

		if (ItemPointerIsValid(&buildstate->odin_tupleid))
		{
			/* insert last record */
			_bt_build_callback(index,
							   &buildstate->odin_tupleid,
							   buildstate->odin_values,
							   buildstate->odin_isnull,
							   buildstate->odin_tuplelive,
							   (void *) buildstate);

			/* TODO: release the last record, both edges */
		}
 ...
 */
static void
odin_build_callback(Relation index,
					ItemPointer tupleId,
					Datum *values,
					bool *isnull,
					bool tupleIsAlive,
					void *state)
{
	...
	int			indnkeyatts = index->rd_index->indnkeyatts;
	bool		sameblock = false;
	int			tuples_per_range = OdinGetTuplesPerRange(index);

	if (tuples_per_range > 0 && RelationIsAppendOptimized(buildstate->heap))
	{
		if (!ItemPointerIsValid(&buildstate->odin_tupleid))
		{
			/* this tuple is the first one of the first record */
			buildstate->odin_count = 1;
			sameblock = false;
		}
		else if (++buildstate->odin_count == tuples_per_range ||
				 (ItemPointerGetBlockNumber(tupleId) !=
				  ItemPointerGetBlockNumber(&buildstate->odin_tupleid)))
		{
			/* this tuple is the first one of the next record */
			buildstate->odin_count = 1;
			sameblock = false;
		}
		else
		{
			/* do not update odin_count, it's already increased in the if */
			sameblock = true;
		}
	}
	else
		sameblock = (ItemPointerGetBlockNumber(tupleId) ==
					 ItemPointerGetBlockNumberNoCheck(&buildstate->odin_tupleid));

	if (sameblock)
	{
		/* the tuple belongs to the same block, record it as the end edge */

		/* FIXME: how to prevent duplicating every tuple? */
		/* TODO: release the previous record, end edge only */
		for (int i = 0; i < indnkeyatts; i++)
		{
			buildstate->odin_isnull[indnkeyatts + i] = isnull[i];

			if (isnull[i])
				buildstate->odin_values[indnkeyatts + i]
					= PointerGetDatum(NULL);
			else
				buildstate->odin_values[indnkeyatts + i]
					= datumCopy(values[i],
								idesc->attrs[i].attbyval,
								idesc->attrs[i].attlen);
		}
	...
	ItemPointerCopy(tupleId, &buildstate->odin_tupleid);
	/* the record tuple is always alive */
	buildstate->odin_tuplelive = true;

	/* this is the first tuple of a block, record it */
	for (int i = 0; i < indnkeyatts; i++)
	{
		buildstate->odin_isnull[i] = isnull[i];
		buildstate->odin_isnull[indnkeyatts + i] = isnull[i];

		/*
		 * mark the tuple as both begin & end, in case it's the only one in the
		 * record.
		 */
		if (isnull[i])
		{
			buildstate->odin_values[i] = PointerGetDatum(NULL);
			buildstate->odin_values[indnkeyatts + i]
				= PointerGetDatum(NULL);
		}
		else
		{
			buildstate->odin_values[i] = datumCopy(values[i],
												   idesc->attrs[i].attbyval,
												   idesc->attrs[i].attlen);
			buildstate->odin_values[indnkeyatts + i]
				= datumCopy(values[i],
...
_bt_pagestate(BTWriteState *wstate, uint32 level)
{
	...
	{
		ODIN_FEATURE_BEGIN(ALL, RelationGetRelid(wstate->index));

		state->btps_full = OdinGetFillFactor(wstate->index);
						   ...
						   Sharedsort *sharedsort2, int sortmem, bool progress)
{
	...
	ODIN_FEATURE_BEGIN(ALL, RelationGetRelid(btspool->index));
	ItemPointerSetInvalid(&buildstate.odin_tupleid);
	buildstate.odin_count = 0;
	ODIN_FEATURE_END(ALL);
	...
	reltuples = table_index_build_scan(btspool->heap, btspool->index, indexInfo,
									   true, progress, odin_build_callback,
									   (void *) &buildstate, scan);

	if (ItemPointerIsValid(&buildstate.odin_tupleid))
	{
		/* insert last record */
		_bt_build_callback(btspool->index,
						   &buildstate.odin_tupleid,
						   buildstate.odin_values,
						   buildstate.odin_isnull,
						   buildstate.odin_tuplelive,
