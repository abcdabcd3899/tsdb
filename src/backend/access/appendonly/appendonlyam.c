...
SetNextFileSegForRead(AppendOnlyScanDesc scan)
{
	...
	if (!scan->initedStorageRoutines)
	{
		...
		fns = mx_get_shared_compression_functions(scan->storageAttributes.compressType);
		...
		if (scan->storageRead.compression_functions != NULL)
		{
			...
			cs = mx_get_shared_compression_state(false /* decompress */,
												 scan->storageAttributes.compressType,
												 scan->storageAttributes.compressLevel,
												 scan->usableBlockSize,
...
retry:
	...
	if (scan->rs_base.rs_parallel != NULL)
	{
		...
		scan->storageRead.ao_parallel_scan.current++;

		if (scan->storageRead.ao_parallel_scan.target == 0)
		{
			table_block_parallelscan_startblock_init(scan->rs_base.rs_rd, pbscan);
			scan->storageRead.ao_parallel_scan.target =
				table_block_parallelscan_nextpage(scan->rs_base.rs_rd, pbscan);
		}

		if (scan->storageRead.ao_parallel_scan.current < 
			scan->storageRead.ao_parallel_scan.target + 1)
		{
		...
		/* reset */
		scan->storageRead.ao_parallel_scan.target = 0;
	}
					  ...
					  Snapshot appendOnlyMetaDataSnapshot)
{
...

	{
		...
		/*
		 * aoFetchDesc is allocated with palloc0(), so is ->lastSequence,
		 * otherwise we need to clear it to 0 before calling
		 * ReadLastSequences()
		 */

		nResults = ReadLastSequences(segrelid,
		...
		for (int i = 0; i < nResults; i++)
		{
			...
			aoFetchDesc->lastSequence[segno] = fastSequence->last_sequence;
		}
	}
	...
	fns = mx_get_shared_compression_functions(NameStr(compresstype));
...
appendonly_insert_init(Relation rel, int segno)
{
	...
	if (fns)
	{
		cs = mx_get_shared_compression_state(true /* compress */,
											 NameStr(compresstype),
											 compresslevel,
											 blocksize,
											 RelationGetDescr(rel));
		if (gp_appendonly_verify_write_block == true)
			verifyCs = mx_get_shared_compression_state(false /* decompress */,
													   NameStr(compresstype),
													   compresslevel,
													   blocksize,
													   RelationGetDescr(rel));
