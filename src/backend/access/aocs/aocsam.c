								  ...
								  struct ParallelTableScanDescData *rs_parallel)
{
	...
	for (AttrNumber i = 0; i < num_proj_atts; i++)
	{
		...
		datumstreamread_block(ds[attno], blockDirectory, attno, i, rs_parallel, -1);
	}
...
aocs_getnext(AOCSScanDesc scan, ScanDirection direction, TupleTableSlot *slot)
{
	...
	/*
	 * If the number of projection attrs are fewer than the output slot, we
	 * need to set the tts_isnull, otherwise non-projected attrs will have
	 * undetermined values. This happens when ANALYZE specified only partial
	 * columns.
	 */
	if (slot->tts_tupleDescriptor->natts > scan->columnScanInfo.num_proj_atts)
...
ReadNext:
		...
		for (AttrNumber i = 0; i < scan->columnScanInfo.num_proj_atts; i++)
		{
			...
			DatumStreamRead *dsr = scan->columnScanInfo.ds[attno];
			bool getNewBlock = false;

			err = datumstreamread_advance(dsr);
			Assert(err >= 0);

			if (err > 0)
			{
				/* 
				 * For parallel scan, the first column drive the parallel block
				 * scan, so the rowNum is not sequential for the first column.
				 *
				 * The following columns need to adjust the scan position if the
				 * first column skipped some blocks.
				 */
				if (rowNum != INT64CONST(-1) &&
					dsr->blockFirstRowNum != INT64CONST(-1) &&
					rowNum != dsr->blockFirstRowNum + datumstreamread_nth(dsr))
				{
					/* the rowNum is out of the current block */
					if (rowNum > dsr->blockFirstRowNum + dsr->blockRowCount - 1)
						getNewBlock = true;
					/* the rowNum is still contained in current block */
					else
					{
						while (rowNum > dsr->blockFirstRowNum + datumstreamread_nth(dsr))
						{
							err = datumstreamread_advance(dsr);
							Assert(err > 0);
						}
						Assert(rowNum == dsr->blockFirstRowNum + datumstreamread_nth(dsr));
					}
				}
			}
			else
			...
			/* end of block or we need to switch to another block in parallel mode */
			if (getNewBlock)
			{
				err = datumstreamread_block(dsr, scan->blockDirectory, attno, i, scan->rs_base.rs_parallel, rowNum);
				if (err < 0)
				{
					/*
			 ...
			 */
			datumstreamread_get(dsr, &d[attno], &null[attno]);
				...
				bool *proj)
{
	...
	{
		FormData_gp_fastsequence results[AOTupleId_MultiplierSegmentFileNum];
		int64		nResults;

		/*
		 * aocsFetchDesc is allocated with palloc0(), so is ->lastSequence,
		 * otherwise we need to clear it to 0 before calling
		 * ReadLastSequences()
		 */

		nResults = ReadLastSequences(aocsFetchDesc->segrelid,
									 0, AOTupleId_MultiplierSegmentFileNum - 1,
									 results);

		for (int i = 0; i < nResults; i++)
		{
			Form_gp_fastsequence fastSequence = &results[i];
			int64		segno = fastSequence->objmod;

			aocsFetchDesc->lastSequence[segno] = fastSequence->last_sequence;
		}
	}
