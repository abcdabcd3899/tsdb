								 ...
								 uint32 flags)
{
	...
	found = extractcolumns_from_node(rel, (Node *)targetlist, proj, natts);
	found |= extractcolumns_from_node(rel, (Node *)qual, proj, natts);

	memcpy(projRecheck, proj, natts * sizeof(*projRecheck));
	if (extractcolumns_from_node(rel, (Node *)bitmapqualorig, projRecheck, natts))
	{
		/*
...
aoco_parallelscan_initialize(Relation rel, ParallelTableScanDesc pscan)
{
	...
	bpscan->phs_nblocks = InvalidBlockNumber;
                             ...
                             bool *call_again, bool *all_dead)
{
	...
	if (!aocoscan->aocofetch)
	{
		...
		/* Initiallize the projection info if not yet, assumes the whole row */
		if (!aocoscan->proj)
		{
			int			natts;
...
static void
aoco_index_fetch_extractcolumns(IndexFetchTableData *scan,
								List *targetlist,
								List *qual)
{
	IndexFetchAOCOData *aocoscan = (IndexFetchAOCOData *) scan;
	...
	Assert(!aocoscan->aocofetch);
	Assert(!aocoscan->proj);

	aocoscan->proj = palloc0(natts * sizeof(*aocoscan->proj));

	found |= extractcolumns_from_node(aocoscan->xs_base.rel, (Node *) targetlist, aocoscan->proj, natts);
	found |= extractcolumns_from_node(aocoscan->xs_base.rel, (Node *) qual, aocoscan->proj, natts);
...
static const TableAmRoutine ao_column_methods = {
	...
	/*
	 * GPDB_12_MERGE_FIXME: it is needed to extract the column information for
	 * fetches before calling index_fetch_tuple().
	 */
	.index_fetch_extractcolumns = aoco_index_fetch_extractcolumns,
};
