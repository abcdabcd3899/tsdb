...
static void gp_print_path(StringInfo msg, Path *path, bool accept_new, int indent);
...
add_path(RelOptInfo *parent_rel, Path *new_path)
{
	...
	if (gp_debug_add_path)
		gp_print_add_path(parent_rel, new_path, accept_new);
...
add_partial_path(RelOptInfo *parent_rel, Path *new_path)
{
	...
	if (new_path->parallel_workers == 0)
		return;
	...
	AssertImply(!CdbPathLocus_IsBottleneck(new_path->locus),
				new_path->parallel_workers == new_path->locus.parallel_workers);
				  ...
				  bool partial_path)
{
	...
	pathnode->path.locus = cdbpathlocus_from_baserel(root, rel, pathnode->path.parallel_workers);
				   ...
				   List *partitioned_rels, double rows)
{
	...
	/* 
	 * FIXME: This part of codes are ungly, need more thought
	 */
	if (parallel_aware)
	{
		/* parallel append is invalid for non partitioned locus */
		if (Gp_role == GP_ROLE_DISPATCH &&
			!CdbPathLocus_IsPartitioned(pathnode->path.locus))
		{
			pathnode->path.parallel_aware = false;	
			pathnode->path.parallel_workers = 0;
		}
		...
		{
			pathnode->path.locus.parallel_workers = parallel_workers;
		}
	}
	...
	foreach(l, pathnode->subpaths)
	{
		...
		pathnode->path.motionHazard = pathnode->path.motionHazard ||
			subpath->motionHazard;
...
#ifdef MATRIXDB_VERSION
		/*
		 * The append node will be discarded later, if the child is not
		 * parallel aware, it may generate duplicated tuples
		 */
		if (pathnode->path.parallel_aware && !child->parallel_aware)
		{
			pathnode->path.startup_cost = DBL_MAX;
			pathnode->path.total_cost = DBL_MAX;
		}
#endif
	}
	else
					  ...
					  List *pathkeys)
{
	...
	*subpaths_out = NIL;
	...
	{
		bool		isfirst = true;
		bool		compatible = true;	/* true if all the subpaths have
										 * compatible distributed policies */
		...
		foreach(l, subpaths)
		{
				...
				CdbPathLocus_IsSegmentGeneral(subpath->locus))
			{
				...
				CdbPathLocus_MakeStrewn(&projectedlocus, numsegments, subpath->parallel_workers);
			}
			else
			{
			...
			{
				/* previous subpaths are already incompatible */
				maxnumsegments = Max(maxnumsegments,
			...
			else
			{
				 ...
				 *
				 * but at the moment we have not walked all the subpaths so the
				 * max numsegments is not known yet, so only set a flag.
				 */
				compatible = false;
				maxnumsegments = Max(CdbPathLocus_NumSegments(targetlocus),
									 CdbPathLocus_NumSegments(projectedlocus));
			}
		}
		...
		if (!compatible)
		{
			...
			CdbPathLocus_MakeStrewn(&targetlocus, maxnumsegments,
		...
		if (CdbPathLocus_IsPartitioned(targetlocus))
		{
				...
				CdbPathLocus_IsSegmentGeneral(subpath->locus))
			{
				...
				CdbPathLocus_MakeStrewn(&(subpath->locus),
				                        numsegments, subpath->parallel_workers);
			}
						 ...
						 Relids required_outer, double *rows)
{
	...
	/* Gather node has same locus with subpath except that paraller_worker is 0 */
	pathnode->path.locus = subpath->locus;
					   ...
					   Relids required_outer)
{
...

	{
		...
		else if (exec_location == PROEXECLOCATION_ALL_SEGMENTS)
		{
			 ...
			 */
			CdbPathLocus_MakeStrewn(&pathnode->locus,
									getgpsegmentCount(), pathnode->parallel_workers);
		}
		else
			CdbPathLocus_MakeGeneral(&pathnode->locus);
	}
					  ...
					  List *dqa_expr_lst)
{
	...
	CdbPathLocus_MakeStrewn(&pathnode->path.locus,
							subpath->locus.numsegments, pathnode->path.parallel_workers);
						 ...
						 double numGroups)
{
	...
	if (CdbPathLocus_IsPartitioned(subpath->locus) &&
            (DO_AGGSPLIT_SERIALIZE(aggsplit) || has_empty_set))
		CdbPathLocus_MakeStrewn(&pathnode->path.locus,
						   ...
						   double numGroups)
{
	...
	pathnode->path.motionHazard =
		leftpath->motionHazard || rightpath->motionHazard;
...
static void
gp_print_path(StringInfo msg, Path *path, bool accept_new, int indent)
{
	const char *ptype;
	bool		join = false;
	Path	   *subpath = NULL;
	...
	switch (nodeTag(path))
	{
		...
		case T_UpperUniquePath:
			...
			break;
		case T_AggPath:
			strategy = ((AggPath *) path)->aggstrategy;
			if (DO_AGGSPLIT_SERIALIZE(((AggPath *) path)->aggsplit))
			{
				if (strategy == AGG_HASHED)
					ptype = "Partial HashAgg";
				else
					ptype = "Partial Grouping Agg";
			}
			else if (DO_AGGSPLIT_COMBINE(((AggPath *) path)->aggsplit))
			{
				if (strategy == AGG_HASHED)
					ptype = "Final HashAgg";
				else
					ptype = "Final Grouping Agg";
			}
			else
			{
				if (strategy == AGG_HASHED)
...
#define TERM_COLOR_PURPLE "\033[0;35m"
#define TERM_COLOR_END  "\033[0m"
	{
		static char *color_pool[2][2] = {
			{TERM_COLOR_GREEN, TERM_COLOR_RED},
			{TERM_COLOR_CYAN, TERM_COLOR_PURPLE}
		};

		int8 is_parallel = path->parallel_workers > 0 ? 1 : 0;
		int8 accept      = accept_new ? 0 : 1;
		char *color      = color_pool[is_parallel][accept];

		appendStringInfo(msg, "%s", color);
	}
	...
	if (join)
	{
		...
		gp_print_path(msg, jp->outerjoinpath, true /* accept_new */, indent + 1);
		appendStringInfo(msg, "\n");
		gp_print_path(msg, jp->innerjoinpath, true /* accept_new */, indent + 1);
	}

	appendStringInfo(msg, "\n");

	appendStringInfo(msg, TERM_COLOR_END);

	if (subpath)
	{
		gp_print_path(msg, subpath, true /* accept_new */, indent + 1);
	}
	else if (nodeTag(path)==  T_AppendPath)
	{
#define MAXDEBUGAPPENDSUBPATHS 3
		struct	ListCell *lc;
		int		i = 0;
		foreach(lc, ((struct AppendPath *)path)->subpaths)
		{
			if (i < MAXDEBUGAPPENDSUBPATHS)
				gp_print_path(msg, (Path *)lfirst(lc), true /* accept_new */, indent + 1);
			else
			{
				for (i = 0; i < indent + 1; i++)
					appendStringInfo(msg, "\t");
				i = list_length(((struct AppendPath *)path)->subpaths);
				appendStringInfo(msg, "... Hidden %d subpaths ...",
								 i - MAXDEBUGAPPENDSUBPATHS);
				break;
			}
			i++;
		}
...
void
gp_print_add_path(RelOptInfo *parent_rel, Path *new_path, bool accept_new)
{
	int			new_pathlen;		
	char		*partial;
	StringInfo	msg = makeStringInfo();

	gp_print_path(msg, new_path, accept_new, 1);

	if (new_path->parallel_workers > 0)
	{
		partial = "Partial";
		new_pathlen = accept_new ? list_length(parent_rel->pathlist) + 1 :
	...
	ereport(WARNING, 
			(errcode(ERRCODE_CHECK_VIOLATION),
			 errmsg("Adding %s paths, path length: %d", partial, new_pathlen),
