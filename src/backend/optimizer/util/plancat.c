				  ...
				  RelOptInfo *rel)
{
	...
	if (rel->amhandler == AO_ROW_TABLE_AM_HANDLER_OID ||
						  ...
						  Relation relation, bool inhparent)
{
	 ...
	 *
	 * In Matrixdb, for a partitioned table, all its members normally have
	 * equivalent FK constraints with the parent, so give it a chance to
	 * make useful FK-based deductions.
	 */
	if (inhparent && relation->rd_rel->relkind != RELKIND_PARTITIONED_TABLE)
		return;
	...
	foreach(lc, cachedfkeys)
	{
		...
		foreach(lc2, rtable)
		{
			...
			if (rte->rtekind != RTE_RELATION ||
				...
				continue;
			/* Ignore if it's an inheritance parent, try partitioned table anywhere */
