			  ...
			  Bitmapset *scan_params)
{
	 ...
	 *
	 * FIXME: parallel-aware plan node is not always generated, so skip the
	 * "parallel-aware plan node is not below a Gather" check here, will it
	 * causes a trouble? have no idea for now.
	 */
