...
static Unique *create_upper_unique_plan(PlannerInfo *root, UpperUniquePath *best_path,
										int flags);
static Agg *create_agg_plan(PlannerInfo *root, AggPath *best_path, int flags);
 ...
 */
static Agg *
create_agg_plan(PlannerInfo *root, AggPath *best_path, int flags)
{
	Agg		   *plan;
	Plan	   *subplan;
	List	   *tlist;
...
create_motion_plan(PlannerInfo *root, CdbMotionPath *path)
{
	...
	/*
	 * For Append plan node, in function set_append_references, upstream has an
	 * optimization, which rid of append node if it only contains single sub-node.
	 *
	 * 	if (list_length(aplan->appendplans) == 1)
	 *      return clean_up_removed_plan_level((Plan *) aplan,
	 *								   (Plan *) linitial(aplan->appendplans));
	 *
	 *  For our motion node, we do not have parallel_aware policy. If we follow
	 *  sub path's parallel_workers, when we meet this append node optimization,
	 *  the input data will be multiple fetched.
	 *
	 *  Pattern-recognized append node, If matched, follow append sub-node
	 *  parallel_workers.
	 */
	int candidate_workers = subpath->parallel_workers;
	if (IsA(subpath, AppendPath))
	{
		AppendPath *appendPath = (AppendPath *)subpath;
		if (list_length(appendPath->subpaths) == 1)
		{
			Path *sub_sub_path = (Path *)linitial(appendPath->subpaths);
			candidate_workers = sub_sub_path->parallel_workers;
		}
	}
	...
	switch (subpath->locus.locustype)
	{
		...
		case CdbLocusType_SegmentGeneral:
			...
			sendSlice->segindex = gp_session_id % subpath->locus.numsegments;
			sendSlice->parallel_workers = candidate_workers;
			break;
	...
	motion->sender_parallel_workers = candidate_workers;
	...
	if (motion->sender_parallel_workers)
		root->glob->parallelModeNeeded = true;
	return (Plan *) motion;
}	/* create_motion_plan */
								 ...
								 Plan *subplan)
{
	 ...
	 */
	int			numHashSegments;
	numHashSegments = cdbpath_parallel_workers(&path->path);
...
Node *
mars_replace_nestloop_params(PlannerInfo *root, Node *expr)
{
