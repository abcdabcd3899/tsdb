...
int			force_parallel_mode_fake = FORCE_PARALLEL_OFF;
...
static void mx_qp_callback(PlannerInfo *root, void *extra);
...
standard_planner(Query *parse, int cursorOptions, ParamListInfo boundParams)
{
	...
	if (Gp_role == GP_ROLE_DISPATCH)
	{
		...
		if (gp_debug_add_path)
			gp_print_add_path(final_rel, best_path, true);

		/* Also try adding motion on top of the cheapest partial path */
		if (final_rel->partial_pathlist)
		{
			PlanSlice *partial_top_slice;
			partial_top_slice = palloc0(sizeof(PlanSlice));
			partial_top_slice->parentIndex = -1;
			partial_top_slice->sliceIndex = 0;

			Path *partial_path = (Path*) linitial(final_rel->partial_pathlist);
			partial_path = cdbllize_adjust_top_path(root, partial_path, partial_top_slice);

			if (partial_path->total_cost < best_path->total_cost)
			{
				best_path = partial_path;
				top_slice = partial_top_slice;
			}

			if (gp_debug_add_path)
				gp_print_add_path(final_rel, partial_path,
								  best_path == partial_path);
		}
	}
				 ...
				 PlannerConfig *config)
{
	...
	root->force_partial_aggregate_path = force_partial_aggregate_path;
				 ...
				 double tuple_fraction)
{
	...
	else
	{
		 ...
		 */
#ifdef MATRIXDB_VERSION
		current_rel = query_planner(root, mx_qp_callback, &qp_extra);
	 ...
	 */
	if (final_rel->consider_parallel &&
		(root->query_level > 1 || enable_parallel_mode))
	{
		...
		foreach(lc, current_rel->partial_pathlist)
		{
			...
			/*
			 * In Matirxdb, we also consider adding limit or Gather motion
			 * to the partial_pathlist, when the locus of partial path is
			 * partitioned, the partial_pathlist has no different with
			 * pathlist from the view of MPP, so use the same logic with
			 * pathlist to use two-stage LIMIT or gather the data with
			 * MOTION node.
			 */
				...
				(limit_needed(parse) || must_gather))
			{
				...
				if (parse->limitCount && limit_needed(parse) &&
					...
					!contain_volatile_functions(parse->limitCount))
				{
					if (!(IsA(partial_path, SubqueryScanPath)
						  && !partial_path->pathkeys
						  && ((SubqueryScanPath *)partial_path)->subpath->pathkeys))
						partial_path = (Path *)
							create_preliminary_limit_path(root, final_rel,
														  partial_path,
														  parse->limitOffset,
														  parse->limitCount,
														  offset_est, count_est);
				...
				pathkeys =
					cdbpullup_truncatePathKeysForTargetList(saved_pathkeys == NIL ? partial_path->pathkeys : saved_pathkeys,
															make_tlist_from_pathtarget(partial_path->pathtarget));

				CdbPathLocus_MakeSingleQE(&locus, getgpsegmentCount());
				path = cdbpath_create_motion_path(root, partial_path, pathkeys, false, locus);
 ...
 * mx_qp_callback
 *
 * Basically same as standard_qp_callback, but for time_bucket_gapfill volatile
 * expr, we exchange to time_bucket expr depends on matrixts' catalog. We store
 * new group pathkey into PlannerInfo->homo_group_pathkeys. The path key is only
 * used in planner to recognize time_bucket_gapfill order.
 */
static void
mx_qp_callback(PlannerInfo *root, void *extra)
{
	standard_qp_callback(root, extra);

	if (root->group_pathkeys)
	{
		standard_qp_extra *qp_extra = (standard_qp_extra *) extra;
		List	   *homo_tlist = NIL;
		bool		homo_group_pathkey = false;
		ListCell   *lc;

		foreach(lc, root->processed_tlist)
		{
			TargetEntry *tle = lfirst_node(TargetEntry, lc);
			const mono_proc_info *info = NULL;

			if (IsA(tle->expr, FuncExpr))
				info = mx_fetch_monotinic_proc(((FuncExpr *) tle->expr)->funcid);

			if (info && info->equivalence_proc_num)
			{
				FuncExpr   *func;
				...
				func->funcid = ObjectIdGetDatum(info->equivalence_proc[0]);
				func->args = list_truncate(func->args, 2);

				homo_group_pathkey = true;
			}

			homo_tlist = lappend(homo_tlist, tle);
		}

		if (homo_group_pathkey)
		{
			root->mx_homo_group_pathkeys =
				make_pathkeys_for_sortclauses(root,
											  qp_extra->groupClause,
											  homo_tlist);
		}

		list_free(homo_tlist);
	}
}
#endif /* MATRIXDB_VERSION */
							   ...
							   RelOptInfo **partially_grouped_rel_p)
{
...
#ifdef MATRIXDB_VERSION
		/* 
		 * Bug fix: PARTITIONWISE_AGGREGATE_PARTIAL is impossible if multi
		 * phase agg is disabled.
		 */
		else if ((extra->flags & GROUPING_CAN_PARTIAL_AGG) != 0 &&
				 root->config->gp_enable_multiphase_agg)
#else
		else if ((extra->flags & GROUPING_CAN_PARTIAL_AGG) != 0)
#endif
	...
	*partially_grouped_rel_p = partially_grouped_rel;
	...
	{
		/* NOTICE: GATHER only gather the partial paths on segment */
	...
	if (CdbPathLocus_IsPartitioned(cheapest_path->locus))
		num_total_input_rows = cheapest_path->rows *
			cdbpath_parallel_workers(cheapest_path);
	else
		num_total_input_rows = cheapest_path->rows;
							...
							AggSplit aggsplit)
{
	...
	double		dNumGroups;
	List	*havingQual =
		DO_AGGSPLIT_SERIALIZE(aggsplit) ? NIL : (List *) parse->havingQual;
	...
	if (!is_sorted)
	{
		 ...
		 * need to redistribute it. If we are doing the first stage,
		 * redistribution is also unnecessary. 
		 */
	...
	/* 
	 * CAUTION: caller has set dNumGroupsTotal to numGroups per
	 * segment/worker, donot divide it again here
	 */
					 ...
					 double limit_tuples)
{
		...
		input_rel->partial_pathlist != NIL)
	{
								   ...
								   cheapest_partial_path->pathkeys))
		{
			...
			sort_path = (Path *) create_sort_path(root,
											 ordered_rel,
			...
			if (!cheapest_partial_path->motionHazard)
			{
				path = (Path *)
					create_gather_merge_path(root, ordered_rel,
											 sort_path,
											 sort_path->pathtarget,
			...
			/* 
			 * In MatrixDB, we also consider adding partial_pathlist
			 * for ordered_rel, MatrixDB will consider using Gather
			 * Merge Motion on the ordered_rel->partial_pathlist
			 * later.
			 */
			if (sort_path->pathtarget != target)
				sort_path = apply_projection_to_path(root, ordered_rel,
												sort_path, target);

			add_partial_path(ordered_rel, sort_path);
		}
		else
		{
			/*
			 * If cheapest partial path doesn't need a sort, add directly
			 * to the partial_pathlist of ordered_rel.
			 */
			if (cheapest_partial_path->pathtarget != target)
				cheapest_partial_path =
					apply_projection_to_path(root, ordered_rel,
											 cheapest_partial_path, target);

			add_partial_path(ordered_rel, cheapest_partial_path);
		}
	}
							 ...
							 Node *havingQual)
{
	...
	if (parse->groupingSets) 
	{
		...
		if (!list_member(partial_target->exprs, gsetid))
			add_column_to_pathtarget(partial_target, (Expr *)gsetid, groupref);
	}
						  ...
						  GroupPathExtraData *extra)
{
	...
	if (can_sort)
	{
		...
		/*
		 * In MatrixDB, we also consider adding a non-parallel agg
		 * on input's partial_pathlist because we have MOTION node.
		 */
		if (input_rel->partial_pathlist && grouped_rel->consider_parallel &&
			Gp_role == GP_ROLE_DISPATCH)
		{
			cheapest_partial_path = linitial(input_rel->partial_pathlist);
			candidate = list_make1(cheapest_partial_path);
		}

		candidate = list_concat(candidate, input_rel->pathlist);
		...
		{
			List *partial_agg_candidate = NIL;
			List *final_group_pathkeys = root->group_pathkeys;
			List *final_groupClause= parse->groupClause;

			if (partially_grouped_rel->pathlist)	
				partial_agg_candidate = list_copy(partially_grouped_rel->pathlist);

			if (partially_grouped_rel->partial_pathlist)
				partial_agg_candidate =
					list_concat(partial_agg_candidate, partially_grouped_rel->partial_pathlist);

			/* 
			 * In the final stage of groupingSets, we group by
			 * gsetid + all group columns
			 */
			if (parse->groupingSets)
			{
				/* all group in the rollup can do sort */
				if (bms_is_empty(gd->unsortable_refs))
				{
					List        *grouping_sets_tlist = NIL;
					Index       groupref;
					...
					final_groupClause = lappend(copyObject(parse->groupClause), gsetcl);

					final_group_pathkeys = make_pathkeys_for_sortclauses(root,
																		 final_groupClause,
																		 grouping_sets_tlist);
				}
				else
					partial_agg_candidate = NIL;
			}

			foreach(lc, partial_agg_candidate)
			{
				Path	   *path = (Path *) lfirst(lc);
				double		dNumGroups;
				bool		is_sorted = true;
				...
				/* We always insert a sort node for final stage of groupingSets */
				if (parse->groupingSets)
					is_sorted = false;
				...
				//if (parse->hasAggs)
				{
					path = (Path *) create_agg_path(root,
													...
													path,
													grouped_rel->reltarget,
													final_groupClause ? AGG_SORTED : AGG_PLAIN,
					...
					/* 
					 * For final stage of groupingsets, the GROUPINGSET_ID is in the pathkeys
					 * of the sort path and the AGG path inherited the pathkeys from the
					 * sort path, this is not expected, so we need to adjust the pathkeys
					 * for AGG path.
					 */
					if (parse->groupingSets)
						path->pathkeys = 
	...
	if (can_hash)
	{
		...
		if (parse->groupingSets)
		{
			...
			consider_groupingsets_paths(root, grouped_rel,
										...
										gd, agg_costs, dNumGroupsTotal, AGGSPLIT_SIMPLE);
			/*
			 * Try for a hash-only groupingsets path over unsorted partial input.
			 */
			if (input_rel->partial_pathlist)
			{
				cheapest_path = linitial(input_rel->partial_pathlist);
				consider_groupingsets_paths(root, grouped_rel,
										cheapest_path, false, true,
										gd, agg_costs, dNumGroupsTotal, AGGSPLIT_SIMPLE);
			}
		...
		else
		{
			...
			cheapest_path = cdb_prepare_path_for_hashed_agg(root,
															cheapest_path,
			...
			/* Matrixdb also consider do a hash agg on input_rel->partial_pathlist */
			if (input_rel->partial_pathlist && grouped_rel->consider_parallel &&
				Gp_role == GP_ROLE_DISPATCH)
			{
				cheapest_path = linitial(input_rel->partial_pathlist);
					...
					hashaggtablesize < work_mem * 1024L)
				{
					 ...
					 * since input order won't matter.
					 */
					cheapest_path = (Path *) create_agg_path(root, grouped_rel,
															 cheapest_path,
															 grouped_rel->reltarget,
															 AGG_HASHED,
															 AGGSPLIT_SIMPLE,
					...
					if (cheapest_path->parallel_workers)
						add_partial_path(grouped_rel, cheapest_path);
					else
						add_path(grouped_rel, cheapest_path);
				}
			}
		}
		...
		if (partially_grouped_rel)
		{
			...
			if (partially_grouped_rel->pathlist)	
			{
				cheapest_path = linitial(partially_grouped_rel->pathlist);
				partial_agg_candidate = list_make1(cheapest_path);
			}
				...
				grouped_rel->consider_parallel)
			{
				...
				partial_agg_candidate = lappend(partial_agg_candidate, cheapest_path);
			}

			if (parse->groupingSets)
			{
				/* all groups can do hash */
				if (bms_is_empty(gd->unhashable_refs))
				{
					List        *grouping_sets_tlist = NIL;
					Index       groupref;
					GroupingSetId *gsetid;
...
#ifdef MATRIXDB_VERSION
	PathTarget *partially_grouped_target = NULL;

	/*
	 * MatrixDB: make sure that partially_grouped_target is initialized before
	 * being used.  The if conditions must be kept in sync with the below if's
	 * where the variable is actually used.
	 */
	if (try_mpp_multistage_aggregation ||
		(!extra->partial_costs_set &&
		 !agg_costs->hasNonCombine &&
		 parse->hasAggs))
	{
		if (partially_grouped_rel == NULL)
	...
	/*
	 * MatrixDB: The original logic was to calculate multi-phase agg costs only
	 * on QD, it was assumed that multi-phase aggs should not happen in utility
	 * mode, etc..  However things are different now, multi-phase aggs can
	 * occur in utility mode in mars custom agg scan, maybe also in parallel
	 * mode, their costs are calculated base on these base agg costs, so random
	 * costs are generated.
	 *
	 * So we should prepare these base agg costs in all the modes, but be
	 * careful do not do it when there are non-combine functions, such as
	 * string_agg().
	 */
	if (!extra->partial_costs_set && !agg_costs->hasNonCombine)
	{
		...
		if (parse->hasAggs)
		{
			...
			Assert(partially_grouped_target);
							  ...
							  bool force_rel_creation)
{
...
#ifdef MATRIXDB_VERSION
	else if (root->force_partial_aggregate_path)
		cheapest_total_path = input_rel->cheapest_total_path;
	else
	{
		...
		/* 
		 * if input_rel is not co-located with grouping key, we
		 * can use partial aggregate
		 */
			...
			if (path == cheapest_total_path || is_sorted)
			{
				...
				if (parse->groupingSets)
				{
					consider_groupingsets_paths(root, partially_grouped_rel,
												path, true, can_hash,
												gd, agg_partial_costs, dNumPartialGroups,
												AGGSPLIT_INITIAL_SERIAL);
				}
				else
				//if (parse->hasAggs)
		...
		foreach(lc, input_rel->partial_pathlist)
		{
			...
			if (path == cheapest_partial_path || is_sorted)
			{
				...
				if (parse->groupingSets)
				{
					consider_groupingsets_paths(root, partially_grouped_rel,
												path, true, can_hash,
												gd, agg_partial_costs, dNumPartialPartialGroups,
												AGGSPLIT_INITIAL_SERIAL);
				}
				else
				//if (parse->hasAggs)
    ...
    if (can_hash && cheapest_total_path != NULL)
	{
		...
		{
			consider_groupingsets_paths(root, partially_grouped_rel,
										cheapest_total_path, false, true,
										gd, agg_partial_costs, dNumPartialGroups,
										AGGSPLIT_INITIAL_SERIAL);
		}
		else
