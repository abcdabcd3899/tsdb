...
bool		enable_parallel_mode = false;
			...
			double *rows)
{
	...
	{
		int			numsegments = CdbPathLocus_NumSegments(path->subpath->locus);

		path->path.rows = numsegments > 1 ?
			rel->rows / numsegments : rel->rows;
	}
				  ...
				  double *rows)
{
	...
	if (rows)
		...
		path->path.rows = rel->rows /
			CdbPathLocus_NumSegments(path->subpath->locus);
