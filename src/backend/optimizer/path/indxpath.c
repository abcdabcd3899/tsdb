...
create_index_paths(PlannerInfo *root, RelOptInfo *rel)
{
	...
	if (bitindexpaths != NIL)
	{
		...
		/* TODO:disable parallel bitmap scan because stream bit map does not support parallel */
		if (false && rel->consider_parallel && rel->lateral_relids == NULL)
...
check_index_only(RelOptInfo *rel, IndexOptInfo *index)
{
	...
	if (rel->amhandler != HEAP_TABLE_AM_HANDLER_OID)
		return false;
