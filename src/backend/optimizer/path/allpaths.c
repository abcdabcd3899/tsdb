						  ...
						  RangeTblEntry *rte)
{
	...
	switch (rte->rtekind)
	{
		case RTE_RELATION:
			...
			/*
			 * FIXME: parallel heap/index/bitmap scan is disable for
			 * catalog table and replicated table 
			 */
			if (!rel->cdbpolicy ||
			...
			/* Currently we only support parallel scan on heap and ao/co storage */
			if (rte->relkind == RELKIND_RELATION && rel->amhandler)
			{
				if (rel->amhandler != AO_COLUMN_TABLE_AM_HANDLER_OID &&
					rel->amhandler != AO_ROW_TABLE_AM_HANDLER_OID &&
					rel->amhandler != HEAP_TABLE_AM_HANDLER_OID)
					return;
			}
	...
	/* CDB: also check the rel->upperrestrictinfo */
	if (!is_parallel_safe(root, (Node *) rel->upperrestrictinfo))
		return;
						...
						List *live_childrels)
{
	...
	bool		partial_subpaths_motionHazard = false;
	bool		pa_subpaths_valid;
	bool		nonpartial_subpaths_motionHazard = false;
	...
	foreach(l, live_childrels)
	{
		...
		if (childrel->partial_pathlist != NIL)
		{
			...
			if (cheapest_partial_path->motionHazard)
				partial_subpaths_motionHazard = true;
		...
		if (pa_subpaths_valid)
		{
					  ...
					  cheapest_partial_path->total_cost < nppath->total_cost))
			{
				...
				if (cheapest_partial_path->motionHazard)
					nonpartial_subpaths_motionHazard = true;
				accumulate_append_subpath(cheapest_partial_path,
										  &pa_partial_subpaths,
	...
	if (partial_subpaths_valid && partial_subpaths != NIL)
	{
		 ...
		 */
		if (enable_parallel_append && !partial_subpaths_motionHazard)
		{
		...
		appendpath = create_append_path(root, rel, NIL, partial_subpaths,
										...
										enable_parallel_append && !partial_subpaths_motionHazard,
										partitioned_rels, -1);
	 ...
	 */
	/* CDB: disallow parallel append on non-partial paths, it is easy to hit crash */
	if (pa_subpaths_valid && pa_nonpartial_subpaths != NIL && !nonpartial_subpaths_motionHazard)
	{
		AppendPath *appendpath;
		ListCell   *lc;
		int			parallel_workers = 0;
