...
#ifdef MATRIXDB_VERSION
const mono_proc_info *
mx_fetch_monotinic_proc(Oid procOid)
{
	static bool try_load_monotonic = true;
	ListCell *lc;

	if (!mx_monotonic_procs &&
		try_load_monotonic &&
		!mars_monotonic_function_catalog_init(&try_load_monotonic))
		return NULL;

	foreach(lc, mx_monotonic_procs)
	{
		mono_proc_info *info = (mono_proc_info *)lfirst(lc);

		if (info->proc == procOid)
			return info;
	}
...
/*
 * mars_dimension_validator
 *
 * Check if a MARS relation packaging policy can meets query order requirements.
 * For timebucket packaging policy can only provide integral multiple of defination
 * order (e.g. packaging by 2 mins. Then data can provide 4 mins, 6 mins... tb order,
 * but not 3 mins tb order). Furthermore, if relation have the timestamp dimension related
 * local order, then we can provide timestamp order.
 *
 * return false: can not provide any order.
 *
 * return true: provide an order, and the order expr is store in order_expr.
 */
static bool
mars_dimension_validator(PlannerInfo *root, Relation relation, Index parent_relid,
				MarsDimensionAttr *dimensionAttr, Expr **order_expr)
{
	ListCell   *lc;
	TupleDesc	tupdesc = RelationGetDescr(relation);
	Form_pg_attribute attr =
		&tupdesc->attrs[AttrNumberGetAttrOffset(dimensionAttr->attnum)];
	...
	foreach(lc, root->processed_tlist)
	{
		...
		if (IsA(tle->expr, FuncExpr))
		{
			...
			Const	   *interval;
			Var		   *timestamp;
			const mono_proc_info *mono_info;

			mono_info = mx_fetch_monotinic_proc(funcExpr->funcid);
			if(!mono_info)
				continue;
			...
			interval = linitial(funcExpr->args);
			if (!IsA(interval, Const))
				continue;

			timestamp = lsecond(funcExpr->args);

			if (!IsA(timestamp, Var))
				continue;

			/* check timestamp arg fits our grouping column */
			if (timestamp->varno != parent_relid ||
				timestamp->varattno != dimensionAttr->attnum)
				continue;

			Assert(timestamp->vartype == attr->atttypid &&
				   timestamp->vartypmod == attr->atttypmod &&
				   timestamp->varcollid == attr->attcollation);

			/* check that the interval fits our grouping interval */
			if (dimensionAttr->dimensiontype == MARS_DIM_ATTR_TYPE_WITH_PARAM)
			{
				Interval paramInterval = {
					.time = dimensionAttr->param,
					.day = 0,
					.month = 0,
				};

				Interval *result = (Interval *)DirectFunctionCall2(interval_interval_mod,
																   interval->constvalue,
																   PointerGetDatum(&paramInterval));

				if (result->time != 0 || result->day != 0 || result->month != 0)
					continue;
			}

			if (contain_volatile_functions((Node *)funcExpr))
			{
				FuncExpr *new_func = copyObject(funcExpr);

				new_func->funcid = ObjectIdGetDatum(mono_info->equivalence_proc[0]);
				new_func->args = list_truncate(new_func->args, 2);

				*order_expr = (Expr *)new_func;
			}
			else
				*order_expr = tle->expr;
		}
		...
		{
			Var *varExpr = (Var *) (tle->expr);
			if (varExpr->varno != parent_relid ||
				varExpr->varattno != dimensionAttr->attnum ||
				dimensionAttr->dimensiontype == MARS_DIM_ATTR_TYPE_WITH_PARAM)
				continue;
	...
	/* For grouping col without param, we do not require it in processed_tlist */
	if (*order_expr == NULL && dimensionAttr->dimensiontype == MARS_DIM_ATTR_TYPE_DEFAULT)
	{
		*order_expr = (Expr *) makeVar(parent_relid,
									   dimensionAttr->attnum,
									   attr->atttypid,
									   attr->atttypmod,
									   attr->attcollation,
									   false /* varlevelsup */);
...
/*
 * Derived from build_index_pathkeys():
 *
 * build_index_pathkeys
 *	  Build a pathkeys list that describes the ordering induced by an index
 *	  scan using the given index.  (Note that an unordered index doesn't
 ...
 */
List *
mars_build_pathkeys_list(PlannerInfo *root,
						Index parent_relid,
						Index relid,
						Path *subpath,
						RelOptInfo *rel,
						ScanDirection scandir,
						List *dimension_attrs,
						List *local_order_attrs)
{
	List	   *retval = NIL;
	Relation	relation;
	TupleDesc	tupdesc;
	...
	MarsDimensionAttr *local_dim = local_order_attrs ? linitial_node(MarsDimensionAttr, local_order_attrs) : NULL;
	RangeTblEntry *rte;
	...
	foreach(lc_attr, dimension_attrs)
	{
		Oid			opclass;
		Oid			opfamily;
		Oid			opcintype;
		ListCell   *lc_expr;
		PathKey	   *cpathkey;
		MarsDimensionAttr *dimAttr = lfirst_node(MarsDimensionAttr, lc_attr);
		Form_pg_attribute attr =
			&tupdesc->attrs[AttrNumberGetAttrOffset(dimAttr->attnum)];

		if (!mars_dimension_validator(root,
									  relation,
									  parent_relid,
									  dimAttr,
									  &order_expr))
			/* if orderkey does not appear in processed target list */
		{
			/*
			 * if the attr slicing by range and it is the local order,
			 * we give it a second chance.
			 */
			if (dimAttr->dimensiontype == MARS_DIM_ATTR_TYPE_WITH_PARAM &&
				local_dim &&
				dimAttr->attnum == local_dim->attnum)
			{
				dimAttr = local_dim;
				if (mars_dimension_validator(root,
											 relation,
				...
				{
					/*
					 * The second chance depends on local order, so we can not support
					 * time_bucket liked FuncExpr order(If it can support, it will be supported
					 * by the first chance).
					 */
					if (IsA(order_expr, FuncExpr))
					{
						order_expr = (Expr *) makeVar(parent_relid,
													  dimAttr->attnum,
													  attr->atttypid,
		...
		foreach(lc_expr, subpath->pathtarget->exprs)
		{
			Var		   *subpath_var;
			Var		   *expr_var;
			bool		match;

			if (!IsA(lfirst(lc_expr), Var))
				continue;

			/*
			 * Since we use parent relid to generate our pathkey's Var. For partition table,
			 * child relation need to give a fake varno to make planner happy.
			 */
			subpath_var = lfirst_node(Var, lc_expr);

			if (IsA(order_expr, FuncExpr))
			{
				FuncExpr *funcExpr = (FuncExpr*) order_expr;

				expr_var = lsecond(funcExpr->args);
			}
			else if (IsA(order_expr, Var))
			{
				expr_var = (Var *) order_expr;
			}
			else
				continue;

			subpath_var->varno = parent_relid;
			subpath_var->varnoold = parent_relid;
			match = equal(subpath_var, expr_var);
			subpath_var->varno = relid;
			subpath_var->varnoold = relid;

			if (match)
				break;
		}

		/* finish if can not find orderkey in subpath target list */
		if (lc_expr == NULL)
			goto finish;
		...
		if (!get_opclass_opfamily_and_input_type(opclass,
												 &opfamily, &opcintype))
			elog(ERROR, "cannot get the opfamily and opcintype of opclass %d",
				 opclass);

		/* for time bucket order, the data pathkey can dynamic generate. */
		cpathkey = make_pathkey_from_sortinfo(root,
											  order_expr,
											  ...
											  0 /* sortref */,
											  NULL /* relids */,
											  dimAttr->dimensiontype == MARS_DIM_ATTR_TYPE_WITH_PARAM /* create_it */);
		...
		/*
		 * Since we use local_order to find the order, local_order requires executor dynamic
		 * provides, after that we can not attach further dimension.
		 */
		if (local_dim == dimAttr)
			goto finish;
	}
