			...
			List *options)
{
	...
	/*
	 * COPY_DIRECT: non-distributed table in QD.
	 *
	 * Rel distributed by MASTERONLY, data only store at QD.
	 * So COPY_DIRECT is all we need.
	 */
	if (Gp_role == GP_ROLE_DISPATCH && !cstate->on_segment &&
...
#ifdef MATRIXDB_VERSION
		/*
		 * CopyTo(): copy from local relation to ***.
		 * CopyToDispatch(): copy from dispatchers' call back to ***.
		 *
		 * POLICYTYPE_ENTRY locus means data stored at QD, so use CopyTo() here.
		 */
