...
static void cdbexplain_depositSliceStats(CdbExplain_RecvStatCtx *recvstatctx, int imsgptr);
						  ...
						  struct CdbExplain_ShowStatCtx *showstatctx)
{
	...
	cdbexplain_depositSliceStats(&ctx.recv, 0);
}								/* cdbexplain_localExecStats */
						 ...
						 struct CdbExplain_ShowStatCtx *showstatctx)
{
	...
	for (imsgptr = 0; imsgptr < ctx.nmsgptr; imsgptr++)
		cdbexplain_depositSliceStats(&ctx, imsgptr);
 ...
 */
static void
cdbexplain_depositSliceStats(CdbExplain_RecvStatCtx *recvstatctx, int imsgptr)
{
	...
	CdbExplain_StatHdr		*hdr = recvstatctx->msgptrs[imsgptr];	
	...
	if (!ss->workers)
	{
		...
		ss->nworker = recvstatctx->nmsgptr; 
...
cdbexplain_depositStatsToNode(PlanState *planstate, CdbExplain_RecvStatCtx *ctx)
{
	...
	{
		CdbExplain_DepStatAcc *acc;
		/*
		 * we prefer to use the stats of the plannode which has
		 * the maximal amount of the tuples, otherwise, use the
		 * plannode with the maximal amount of nloops.
		 */
		...
		instr->starttime = acc->nsimax->starttime;
		instr->counter = acc->nsimax->counter;
		instr->firsttuple = acc->nsimax->firsttuple;
		instr->startup = acc->nsimax->startup;
		instr->total = acc->nsimax->total;
		instr->ntuples = acc->nsimax->ntuples;
		instr->ntuples2 = acc->nsimax->ntuples2;
		instr->nloops = acc->nsimax->nloops;
		instr->nfiltered1 = acc->nsimax->nfiltered1;
		instr->nfiltered2 = acc->nsimax->nfiltered2;
		instr->execmemused = acc->nsimax->execmemused;
		instr->workmemused = acc->nsimax->workmemused;
		instr->workmemwanted = acc->nsimax->workmemwanted;
		instr->workfileCreated = acc->nsimax->workfileCreated;
		instr->firststart = acc->nsimax->firststart;
	}
...
cdbexplain_showExecStats(struct PlanState *planstate, ExplainState *es)
{
	 ...
	 */
	if (gp_enable_explain_allstat && ns->ninst > 0)
	{
		if (es->format == EXPLAIN_FORMAT_TEXT)
		{
			/*
