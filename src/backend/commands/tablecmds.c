...
#ifdef MATRIXDB_VERSION
	/* TODO: replace strcmp, we need a private function to recongize MARS */
	else if (stmt->accessMethod && (pg_strcasecmp("mars", stmt->accessMethod) == 0))
	{
		(void) default_reloptions(reloptions, true, RELOPT_KIND_MARS);
	}
#endif
	else
...
#ifdef MATRIXDB_VERSION
		/*
		 * MatrixDB: MARS behaves similar to AOCO in some sense, for example we
		 * allow per-column encoding options, so loose the checks here.
		 */
		if (amHandlerOid == HEAP_TABLE_AM_HANDLER_OID ||
			amHandlerOid == AO_ROW_TABLE_AM_HANDLER_OID)
				...
				ereport(ERROR,
						(errcode(ERRCODE_GP_FEATURE_NOT_YET),
						 errmsg("cannot expand table \"%s\"",
								RelationGetRelationName(rel)),
						 errdetail("MASTERONLY table does not support expansion.")));
			}
#endif /* MATRIXDB_VERSION */
...
#ifdef MATRIXDB_VERSION
	/* For data translate to MARS subpartition, we cannot change the partition range
	 * before data is settle down. Otherwise, we need a lock on root table to block
	 * all access. Using a GUC mx_partition_without_validation internal to implement
	 * WITHOUT VALIDATION semantic.*/
	if ((newrel || needscan) && !mx_partition_without_validation)
#else /* MATRIXDB_VERSION */
	if (newrel || needscan)
#endif /* MATRIXDB_VERSION */
	{
...
#ifdef MATRIXDB_VERSION
	if (RelationIsMars(rel))
		elog(ERROR, "mars: cannot add column to mars table");
#endif /* MATRIXDB_VERSION */
...
#ifdef MATRIXDB_VERSION
	if (RelationIsMars(rel))
		elog(ERROR, "mars: cannot drop column from mars table");
#endif /* MATRIXDB_VERSION */
						  ...
						  bool recurse, bool recursing, LOCKMODE lockmode)
{
	...
	for (i = 0; i < numpks; i++)
	{
		...
		/* In MatrixDB we allow to add a FK on non-primary key */
		if (indexOid == InvalidOid)
			pkopclass = GetDefaultOpClass(pktype, BTREE_AM_OID);
		else
			pkopclass = ObjectIdGetDatum(opclasses[i]);

		/* We need several fields out of the pg_opclass entry */
		cla_ht = SearchSysCache1(CLAOID, pkopclass);
		if (!HeapTupleIsValid(cla_ht))
			elog(ERROR, "cache lookup failed for opclass %u", pkopclass);
					   ...
					   Oid *ppeqoperators, Oid *ffeqoperators, bool old_check_ok)
{
	...
	if (pkrel->rd_rel->relkind == RELKIND_RELATION)
	{
		if (indexOid == InvalidOid)
		{
			if (Gp_role == GP_ROLE_DISPATCH || Gp_role == GP_ROLE_UTILITY)
				ereport(WARNING,
						(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
						 errmsg("Referential integrity constraints are (can) not performed."),
						 errdetail("Adding foreign key on non primary key"),
						 errhint("This FK is used by planner for cost estimation in MatrixDB")));
		}
		else
	...
	if (pkrel->rd_rel->relkind == RELKIND_PARTITIONED_TABLE)
	{
		...
		for (int i = 0; i < pd->nparts; i++)
		{
			...
			/*
			 * MatrixDB allow adding FK on non primary key in which
			 * case a unique index is not needed
			 */
#if 0
		...
		ereport(ERROR,
				(errcode(ERRCODE_UNDEFINED_OBJECT),
				 errmsg("there is no primary key for referenced table \"%s\"",
						RelationGetRelationName(pkrel)),
				 errhint("create a primary key or explicitly specify a referenced column")));
	}
						...
						Oid *opclasses) /* output parameter */
{
	...
	if (!found)
	{
		...
		{
			/* Matrixdb allow FK to refer non-primary key */
			return InvalidOid;
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_FOREIGN_KEY),
...
#ifdef MATRIXDB_VERSION
	if (RelationIsMars(rel))
		elog(ERROR, "mars: cannot alter column type of mars table");
#endif /* MATRIXDB_VERSION */
...
#ifdef MATRIXDB_VERSION
	if (RelationIsMars(rel))
		elog(ERROR, "mars: cannot alter column options of mars table");
#endif /* MATRIXDB_VERSION */
...
#ifdef MATRIXDB_VERSION
		if (ldistro && ldistro->ptype == POLICYTYPE_ENTRY)
		{
			ereport(ERROR,
			   (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				errmsg("not support alter DISTRIBUTED BY to MASTERONLY")));
		}
#endif /* MATRIXDB_VERSION */
	}
...
#ifdef MATRIXDB_VERSION
	if (!mx_partition_without_validation)
