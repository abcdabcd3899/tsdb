			   ...
			   int elevel, gp_acquire_sample_rows_context *ctx)
{
	...
	else /* funny indentation to avoid re-indenting upstream code */
  {
	...
	if (sample_needed)
	{
		...
		if (inh)
			numrows = acquire_inherited_sample_rows(onerel, elevel, va_cols,
													rows, targrows,
													&totalrows, &totaldeadrows);
		else
			numrows = (*acquirefunc) (onerel, elevel, va_cols,
									  rows, targrows,
									  &totalrows, &totaldeadrows);
 ...
 */
int
acquire_sample_rows(Relation onerel, int elevel, List *va_cols,
					HeapTuple *rows, int targrows,
					double *totalrows, double *totaldeadrows)
{
	...
	scan = table_beginscan_analyze(onerel, va_cols);
 ...
 */
int
acquire_inherited_sample_rows(Relation onerel, int elevel, List *va_cols,
							  HeapTuple *rows, int targrows,
							  double *totalrows, double *totaldeadrows)
{
	List	   *tableOIDs;
	...
	for (i = 0; i < nrels; i++)
	{
		...
		if (childblocks > 0)
		{
			...
			if (childtargrows > 0)
			{
				...
				childrows = (*acquirefunc) (childrel, elevel, va_cols,
							   ...
							   double *totalrows, double *totaldeadrows)
{
	...
	GpAcquireSampleRowsStmt *stmt = makeNode(GpAcquireSampleRowsStmt);
	stmt->relOid = RelationGetRelid(onerel);
	stmt->va_cols = va_cols;
	stmt->targrows = perseg_targrows;
	stmt->inherited = inh;
		...
		for (int rowno = 0; rowno < PQntuples(pgresult); rowno++)
		{
			...
			else
			{
				...
				for (i = 0; i < relDesc->natts; i++)
				{
					...
					if (PQgetisnull(pgresult, rowno, index + 3))
						values[i] = NULL;
					else
						values[i] = PQgetvalue(pgresult, rowno, index + 3);
