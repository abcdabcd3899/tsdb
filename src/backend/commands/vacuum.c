					...
					bool isvacuum)
{
	...
	/*
	 * For upstream based FDW, we can not treat it as mpp relation, skip
	 * gpdb private code.
	 */
    if (!ISFDW(relation) || ISMPPFDW(relation))
    {
		/*
		...
		if (num_pages < 1.0)
		{
			...
			if (num_tuples >= 1.0)
			{
				...
				/* 
				 * In matrixdb, the index relation might be empty, it can
				 * refers to an outside index storage.
				 * */
...
#ifdef MATRIXDB_VERSION
	if (RelationIsMars(onerel))
		elog(ERROR, "mars: VACUUM is not needed for mars tables");
#endif /* MATRIXDB_VERSION */
