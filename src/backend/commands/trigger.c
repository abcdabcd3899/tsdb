...
#ifdef MATRIXDB_VERSION
	/*
	 * PG has only one buildin am HEAPAM, so the ExecFetchSlotHeapTuple above
	 * either return a heap tuple already exist in the slot or form a new heap
	 * tuple and store it in the slot (which will be cleaned later).
	 *
	 * In Matrixdb/GPDB, we have more ams like AO/AOCO/MARS which all have the
	 * slot_callbacks as TTSOpsVirtual. So the ExecFetchSlotHeapTuple above
	 * will form a new heap tuple and don't store it in the slot (which will
	 * not be explictly cleaned later). For such case, we decide to explictly
	 * clean the new tuple to avoid memory leak.
	 */
	if (newtuple && should_free && slot->tts_ops->get_heap_tuple == NULL)
