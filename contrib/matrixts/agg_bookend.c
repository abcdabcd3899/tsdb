...
PG_FUNCTION_INFO_V1(ts_first_sfunc);
PG_FUNCTION_INFO_V1(ts_first_combinefunc);
PG_FUNCTION_INFO_V1(ts_last_sfunc);
PG_FUNCTION_INFO_V1(ts_last_combinefunc);
PG_FUNCTION_INFO_V1(ts_bookend_finalfunc);
PG_FUNCTION_INFO_V1(ts_bookend_serializefunc);
PG_FUNCTION_INFO_V1(ts_bookend_deserializefunc);

PG_FUNCTION_INFO_V1(mx_last_not_null_sfunc);
PG_FUNCTION_INFO_V1(mx_last_not_null_finalfunc);
PG_FUNCTION_INFO_V1(mx_last_not_null_combinefunc);
...
polydatum_deserialize_type(StringInfo buf)
{
	...
	Oid type_oid =
		...
		GetSysCacheOid2(TYPENAMENSP, Anum_pg_type_oid, PointerGetDatum(type_name), ObjectIdGetDatum(schema_oid));
#else
...
inline static void
outfunccache_init(OutFuncCache *cache)
{
	cache->out_type = InvalidOid;
}
...
inline static char *
outfunccache_out(OutFuncCache *cache, const PolyDatum *value)
{
	if (!OidIsValid(cache->out_type))
	{
		bool		varlen;

		if (!OidIsValid(value->type_oid))
			elog(ERROR, "could not determine the type of the element");

		getTypeOutputInfo(value->type_oid, &cache->out_type, &varlen);
		fmgr_info(cache->out_type, &cache->proc);
	}

	return OutputFunctionCall(&cache->proc, value->datum);
}
...
typedef struct TransCache
{
	...
	OutFuncCache value_out_func_cache;
	OutFuncCache cmp_out_func_cache;
} TransCache;
...
transcache_get(FunctionCallInfo fcinfo)
{
	...
	if (my_extra == NULL)
	{
		...
		outfunccache_init(&my_extra->value_out_func_cache);
		outfunccache_init(&my_extra->cmp_out_func_cache);
	}
	return my_extra;
}
...
/*
 * (state internal, value anyelement, time "any") => internal
 */
Datum
mx_last_not_null_sfunc(PG_FUNCTION_ARGS)
{
 ...
 */
Datum
mx_last_not_null_finalfunc(PG_FUNCTION_ARGS)
{
	MemoryContext aggcontext;
	...
	{
		InternalCmpAggStore *store =
			(InternalCmpAggStore *) PG_GETARG_POINTER(0);
		...
		Assert(!store->value.is_null);
		Assert(!store->cmp.is_null);

		/* the result format: '["value","time"]' */
		initStringInfoOfSize(&json, 64);

		appendStringInfoCharMacro(&json, '[');

		str = outfunccache_out(&cache->value_out_func_cache, &store->value);
		escape_json(&json, str);
		pfree(str);

		appendStringInfoCharMacro(&json, ',');

		str = outfunccache_out(&cache->cmp_out_func_cache, &store->cmp);
		escape_json(&json, str);
		pfree(str);

		appendStringInfoCharMacro(&json, ']');

		result = cstring_to_text_with_len(json.data, json.len);
		pfree(json.data);
 ...
 */
Datum
mx_last_not_null_combinefunc(PG_FUNCTION_ARGS)
{
	AssertImply(!PG_ARGISNULL(0), PG_GETARG_POINTER(0));
	...
	return ts_last_combinefunc(fcinfo);
}
