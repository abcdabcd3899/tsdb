 ...
 * useful for MatrixDB. The function info is used in various query
...
initialize_func_info()
{
	...
	Oid extension_nsp = get_namespace_oid("public", false);
	...
	for (i = 0; i < _MAX_CACHE_FUNCTIONS; i++)
	{
		...
		Oid namespaceoid = finfo->is_matrixts_func ? extension_nsp : pg_nsp;
