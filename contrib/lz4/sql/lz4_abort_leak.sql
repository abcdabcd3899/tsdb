...
CREATE TABLE lz4_leak_test (distkey int4, id int4, t text)
WITH (appendonly=true, compresstype=lz4, orientation=column) DISTRIBUTED BY (distkey);
...
--
-- If the LZ4 buffers are leaked, the leak isn't that large, so with a decent
...
create function lz4_error_test(n int) returns int as
$$
declare
  i int;
  nerr int;
begin
  ...
  for i in 1..n loop
    begin
      insert into lz4_leak_test select x, errfunc(x) from foo;
    exception
