...
#define FDW_OPTION_SMARTSCAN "smartscan"
...
static const struct PxfFdwOption valid_options[] = {
	...
	{FDW_OPTION_SMARTSCAN, ForeignTableRelationId},
...
pxf_fdw_validator(PG_FUNCTION_ARGS)
{
	...
	foreach(cell, options_list)
	{
		...
		else if (strcmp(def->defname, FDW_OPTION_SMARTSCAN) == 0)
			smartscan = defGetBoolean(def);
...
PxfGetOptions(Oid foreigntableid)
{
	...
	foreach(lc, options)
	{
		...
		if (strcmp(def->defname, FDW_OPTION_PXF_HOST) == 0)
			...
			opt->smartscan = defGetBoolean(def);
		else if (strcmp(def->defname, FDW_OPTION_FORMAT) == 0)
		{
