...
struct MatrixBypassError *mBypassErr;
/*
...
{
    mBypassErr = palloc0(sizeof(struct MatrixBypassError));
...
pxfBeginForeignScan(ForeignScanState *node, int eflags)
{
	...
	if (rel->exec_location != FTEXECLOCATION_ALL_SEGMENTS || Gp_role == GP_ROLE_DISPATCH)
	{
        ...
        PG_TRY();
        {
            mBypassErr->monitor_dir_not_exist = options->smartscan;
        ...
        PG_CATCH();
        {
            /* do not re-throw the monitor error */
            if (unlikely(!mBypassErr->monitor_error_happend))
                PG_RE_THROW();
            else
            	cache_error = true;
        }
	...
	{
        if (!foreignScan->fdw_private || FdwScanPrivateFragmentList >= list_length(foreignScan->fdw_private))
        {
...
pxfReScanForeignScan(ForeignScanState *node)
{
    ...
    /* Do not reset any if dir is empty */
    if (!mBypassErr->monitor_error_happend &&  node->fdw_state != NULL)
    {
        PxfFdwScanState *pxfsstate = (PxfFdwScanState *) node->fdw_state;
...
pxfEndForeignScan(ForeignScanState *node)
{
    ...
    /* Do not need to recycle any resource if dir is empty */
    if ((node->fdw_state == NULL) || (mBypassErr->monitor_dir_not_exist && mBypassErr->monitor_error_happend))
    {
        mBypassErr->monitor_dir_not_exist = false;
        mBypassErr->monitor_error_happend = false;
