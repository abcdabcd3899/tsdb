...
finish_upload(churl_context *context)
{
	...
	/* 
	 * if the buffer is empty, we can not flush an empty
	 * message to pxf server.
	 */
	if (context->upload_buffer->top == 0)
		return;
...
check_response_code(churl_context *context)
{
	...
	else if (response_code != 200 && response_code != 100)
	{
        ...
        extern struct MatrixBypassError *mBypassErr;
        if (mBypassErr->monitor_dir_not_exist && (strstr(err.data, "Input path does not exist") != NULL)) {
            mBypassErr->monitor_error_happend = true;
        }
